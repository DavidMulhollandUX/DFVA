...
When the venture's core artefact is a quantitative instrument, the act of **operationalising it for external registration forces an exactness the live product can quietly avoid** — and that exactness surfaces latent bugs. Drafting the EXP-04 Step-0 freeze (§10 Phase 3) required declaring the *exact* scoring function before hashing it. Doing so exposed a real inconsistency in the shipped product *(historical: `compass-static` was deleted 2026-07-04; the canonical scoring now lives in `dfva/source/rubric.ts`)*: `compass-static/src/data/programData.ts` declares `maxScore: 36`, but 11 items × max 3 sum to **33 achievable** (the Bachelor of Design dimensions sum to exactly its reported 17), and whether the Irreplaceability *bonus* is additive or weighted was never pinned. The product had carried an off-by-3 denominator silently because nothing had ever forced the question; a pre-registration's freeze did.
