...
- **Correction, 2026-08-15:** the first draft said account data was *all* the personal information held. That was wrong. `uploadAlumniData` is a live submission path writing `AlumniDestination` rows — `programCode`, `graduationYear`, `jobTitle`, `employer`, `industryCluster`. No names and no contact details, but in a small cohort that combination could point at one graduate, so it must be disclosed and treated as sensitive. `ContactFormMessage` also stores free text. Both are now named on `/trust` and in the privacy policy. **Lesson for anyone extending these documents: enumerate the write paths in `operations.ts`, don't reason from the headline feature.**
