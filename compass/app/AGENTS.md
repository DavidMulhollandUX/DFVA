...
**Tech stack:** Wasp 0.24 (main.wasp.ts Wasp Spec, Node ≥24.14), React 19, TypeScript, Prisma (PostgreSQL), Tailwind CSS
...
- `wasp db migrate-dev`'s "Can not connect to database" check can falsely fail (wasp quirk since 0.22); the server connects fine. For migrations prefer `DATABASE_URL=… npx prisma migrate dev --schema .wasp/out/db/schema.prisma`. If `prisma migrate status` is up to date, `wasp start` is sufficient.

## Rules
- Use conventional commits (feat:, fix:, refactor:, docs:, chore:)
- Test before committing. Lint before pushing. `npm run check` (prettier, eslint, tsc, vitest) is what CI runs — run it before every commit.
