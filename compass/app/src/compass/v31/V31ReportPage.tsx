...
function MiniMatrix({
  ...
  return (
    <svg
      ...
      className="w-full"
      role="img"
      aria-label="v3.1 exposure–adaptiveness matrix with exact envelope"
    >
      <rect
        x={PAD}
        y={PAD}
...
function PositionLabel({ stability }: { stability: V31Stability }) {
  ...
  if (m >= 0.6 && stability.runnerUpQuadrant) {
    return (
      ...
      >
        {LABELS[stability.modalQuadrant].measured} /{" "}
        {LABELS[stability.runnerUpQuadrant].measured} — boundary case
      </span>
    );
  }
  return (
...
export default function V31ReportPage() {
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-24 text-center">
        ...
        <p className="text-muted-foreground mb-6">
          No v3.1 assessment exists for “{code}” — v3.1 covers the{" "}
          {V3_META.placed} placed programs.
        </p>
        <Link
          to="/insights"
  ...
  const confidenceLabel =
    s.stabilityClass === "boundary" ? "near a threshold" : "firm";
  ...
  return (
    <InsightsGate>
      <div className="mx-auto max-w-5xl px-4 py-10">
          ...
          <CardContent className="pt-6">
            {program.cohort === "extension" && (
              <div className="bg-card-accent text-muted-foreground mb-5 flex items-start gap-2 rounded-md p-3 text-sm">
                ...
                <span>
                  This program was assessed after the reference portfolio was
                  fixed. Its exposure is computed by the identical procedure,
                  but {program.nExtension} of its {program.nTitles} destination
                  titles were crosswalked in a later pass, and its position is
                  measured against the reference portfolio's thresholds rather
                  than re-basing them. {program.extensionNote}
                </span>
              </div>
            )}
              ...
              <div>
                <div className="flex flex-wrap items-baseline gap-x-8 gap-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground text-xs">
                      median {V3_META.adaptMedian} · under ±1 rating error the
                      score could reach {s.adaptEnvelope[0]}–
                      {s.adaptEnvelope[1]}
                    </p>
                  </div>
                  <div>
                ...
                <p
                  ...
                  data-testid="sensitivity-strip"
                >
                  There is a{" "}
                  <strong className="text-foreground">
                    {pct(s.modalProbability)} chance
                  </strong>{" "}
                  this program's position is the one shown, given the assumed
                  rater-error rate (no rater study has been run — under an
                  optimistic assumption {pct(s.modalProbabilityOptimistic)},
                ...
                <div className="bg-card-accent text-muted-foreground mt-4 flex items-start gap-2 rounded-md p-3 text-sm">
                  ...
                  <span data-testid="exposure-explainer">
                    ...
                    much that occupation's tasks overlap with what current AI
                    can do, rescaled 0–100. A value of{" "}
                    {program.exposure.toFixed(2)} says a large share of the
                    tasks in this program's destination occupations overlap with
                    AI capability. It does{" "}
                    <strong className="text-foreground">not</strong> mean those
                    jobs are disappearing — across the Australian labour market,
                    the most AI-exposed occupations are projected to grow,
                    because exposed work tends to be skilled work. Exposure
                    indicates where the <em>content</em> of work is likely to
                    change; what that change means for graduates depends on the
                    adaptiveness axis.
                  </span>
                </div>
                ...
                {s.stabilityClass === "boundary" && (
                  <div className="bg-card-accent text-muted-foreground mt-3 flex items-start gap-2 rounded-md p-3 text-sm">
                    ...
                    <span>
                      This program sits{" "}
                      {Math.abs(s.distToAdaptMedian) === 0
                        ? "exactly at"
                        : `${Math.abs(s.distToAdaptMedian)} point from`}{" "}
                ...
                {Math.abs(program.exposure - V3_META.expMedian) <= 2.5 && (
                  <div className="bg-card-accent text-muted-foreground mt-3 flex items-start gap-2 rounded-md p-3 text-sm">
                    ...
                    <span data-testid="exposure-proximity-note">
                      <strong className="text-foreground font-medium">
                        Near the exposure median (
                        {Math.abs(program.exposure - V3_META.expMedian).toFixed(
                          2,
                        )}{" "}
                        AIOE away).
                      </strong>{" "}
                      The precision analysis models rating error on the scored
                      axis only — exposure carries systematic crosswalk
                      uncertainty that is disclosed, not distributed. A program
                      this close to the exposure threshold has quadrant
                      uncertainty these probabilities do not capture.
                    </span>
                  </div>
                )}
                ...
                {s.nearDisplayThreshold && (
                  <div className="bg-card-accent text-muted-foreground mt-3 flex items-start gap-2 rounded-md p-3 text-sm">
                    ...
                    <span data-testid="near-threshold-note">
                      <em>
                        This label sits near the display threshold (
                        {pct(s.modalProbability)} against an 80% rule). At a
                        pessimistic rater-error assumption (
                        {pct(s.modalProbabilityPessimistic)}) it would be
        ...
        <Card className="mb-6">
          <CardContent className="pt-6">
            ...
            <CardTitle className="text-lg">
              Where the defences are, and aren't
            </CardTitle>
            <p className="text-muted-foreground mt-1 mb-6 text-sm">
              Scored from curriculum evidence, 0–3 per dimension. Tap a
              dimension to see how the four levels are anchored and the evidence
              behind this program's score. The precision analysis in Part C
              quantifies what a ±1 rating difference on these items would do to
              the position.
            </p>
        ...
        <p className="text-muted-foreground mb-5 text-sm">
          Everything the position claims, with its assumptions exposed.
          Precision in the computation must not be mistaken for accuracy in the
          model — the sections below say exactly which numbers rest on which
          assumptions.
        </p>

        <MethodDetails
          summary={`Rater-error sensitivity — there is no rater study yet, and the assumed error rate decides the headline (this program: ${pct(
            s.modalProbability,
          )}, ${pct(s.modalProbabilityOptimistic)} optimistic, ${pct(
          ...
          <Card className="border-0 shadow-none">
            <CardContent className="pt-4">
              <p className="text-muted-foreground mb-4 text-sm">
                No inter-rater study has been run (open item R9), so the
                per-item error rate e is an <em>assumption</em>, not a
                measurement. Publishing only one value would present a choice of
                assumption as a property of the portfolio — so all three are
                published.
              </p>
                ...
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr>
                      {[
                        "Error rate",
                        "This program's stability",
                        "Programs failing the single-label rule (portfolio)",
                      ].map((h) => (
                        <th
                          key={h}
                  ...
                  <tbody>
                    {(
                      [
                        [
                          "e = 0.05 (optimistic)",
                          s.modalProbabilityOptimistic,
                          V31_META.failSingleLabel.optimistic,
                        ],
                        [
                          "e = 0.10 (published assumption)",
                          s.modalProbability,
              ...
              <div className="bg-card-accent text-muted-foreground mt-4 flex items-start gap-2 rounded-md p-3 text-sm">
                <span className="text-base">⚠</span>
                <span>
                  If raters agree at e ≈ 0.20, then{" "}
                  {V31_META.failSingleLabel.pessimistic} of {V3_META.placed}{" "}
                  programs — not {V31_META.failSingleLabel.published} — fail the
                  single-label rule, and the current report understates
                  instability for a large share of the placed portfolio.
                </span>
              </div>
            </CardContent>
          </Card>
        ...
        >
          <Card className="border-0 shadow-none">
            <CardContent className="pt-4">
              <p className="text-muted-foreground mb-4 text-sm">
                The 34 exact stability values form two well-separated clusters —{" "}
                {V31_META.boundaryCount} near-threshold (0.799–0.848) and{" "}
                {V31_META.stableCount} firm (0.979–1.000) — with a 0.13-wide
                empty band between them that no program occupies. The cause is
                distance from the adaptiveness median and nothing else: one
                point away, a single item's error can cross the threshold; two
                points away needs two coincident errors.
              </p>
              <div className="grid grid-cols-2 gap-3 text-center sm:grid-cols-4">
                ...
                <div className="bg-card-accent rounded-md p-3">
                  ...
                  <p className="text-muted-foreground text-xs">
                    Points from adaptiveness median
                  </p>
                </div>
                <div className="bg-card-accent rounded-md p-3">
                  <p className="font-mono text-xl font-semibold">
                  ...
                  <p className="text-muted-foreground text-xs">
                    Items at ceiling (3/3)
                  </p>
                </div>
                <div className="bg-card-accent rounded-md p-3">
                  <p className="font-mono text-xl font-semibold">
                    {V31_META.stabilityClassCut}
                  </p>
                  <p className="text-muted-foreground text-xs">
                    Class cut (any value in 0.85–0.98 identical)
                  </p>
                </div>
              </div>
              <div className="bg-card-accent text-muted-foreground mt-4 flex items-start gap-2 rounded-md p-3 text-sm">
                ...
                <span>
                  <strong className="text-foreground font-medium">
                    Error-model asymmetry.
                  </strong>{" "}
                  Perturbation clamps at the 0–3 item bounds, so 53 of 170
                  scored items (31%) at ceiling can only be perturbed downward —
                  an expected net drift of −0.075 adaptiveness points
                  concentrated in high-adaptiveness programs. A symmetric
                  alternative was tested: maximum stability difference 0.0057,
                  zero label-regime changes. Clamping is retained as the simpler
                  rule.
                </span>
              </div>
            </CardContent>
        ...
        <MethodDetails summary="What changed from v3 — exact enumeration replaced sampling; the numbers are the same, their standing is not">
          <Card className="border-0 shadow-none">
            <CardContent className="pt-4">
              <p className="text-muted-foreground mb-4 text-sm">
                v3.1 is a correctness amendment to the uncertainty layer only —
                no construct, panel, data source or exposure value changes.
                Exact enumeration agrees with v3's 20,000-draw estimate to a
                maximum deviation of 0.0038; v3's published dual-label count of
                2 was correct, but was one draw from a seed-dependent
                distribution over
                {" {0, 1, 2}"}.
              </p>
              <div className="overflow-x-auto">
                    ...
                    {(
                      [
                        [
                          ...
                          "20,000-draw Monte-Carlo, seeded RNG",
                          "Exact enumeration of all 243 states — deterministic, seedless",
                        ],
                        [
                          "Published count of boundary cases",
                          "Seed-dependent (0, 1 or 2 across seeds)",
                          "Exact: 2 programs below the 80% rule at e = 0.10",
                        ],
                        [
                          "Error-rate assumption",
                          "Single rate e = 0.10, implicit",
                          `Three rates published (0.05 / 0.10 / 0.20); headline moves ${V31_META.failSingleLabel.optimistic} → ${V31_META.failSingleLabel.published} → ${V31_META.failSingleLabel.pessimistic} of ${V3_META.placed}`,
                        ],
                        [
                          "Adaptiveness interval",
                          "Observed range across draws",
                          `Exact reachable envelope (${s.adaptEnvelope[0]}–${s.adaptEnvelope[1]} for this program)`,
                        ],
                        [
                          "Stability summary",
                          "Modal probability only",
                          `Position-confidence class from the empirical empty band (this program: ${confidenceLabel})`,
                        ],
                        [
                          "Near-threshold disclosure",
                          "None",
                          "Mandatory note within 0.02 of a display threshold",
                        ],
                        [
                          "Clamping asymmetry",
                          "Implicit",
                          "Disclosed: 31% of items at ceiling, net drift −0.075, label-equivalent to symmetric alternative",
                        ],
        ...
        <div className="text-muted-foreground border-border mt-12 flex flex-wrap items-center justify-between gap-3 border-t pt-6 text-xs">
          <span>
            Evidura · Durability Assessment · v3.1 · exact enumeration, assumed
            rater-error rate {V31_META.errorRates.published}
          </span>
