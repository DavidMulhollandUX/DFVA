      ...
      <div className="mx-auto max-w-7xl px-4 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-3 max-w-3xl text-sm leading-relaxed">
            ...
            curriculum builds the capabilities that hold up under that
            exposure.{" "}
            ...
            <CardContent className="space-y-4 pt-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm">
                  ...
                  measured, not rated, so "one rating difference" is
                  meaningless there.
                </p>
              </div>
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm">
                  ...
                  program regardless of its item scores, and{" "}
                  {
                    stats.gates.filter(
                      (r) =>
                        r.position !== null && r.position !== "attention",
                    ).length
                  }{" "}
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-2">
          <Card>
            ...
            <CardContent>
              ...
              {[["Adaptive items (/15)", stats.adaptiveBars], ["Workplace items (/9)", stats.workplaceBars]].map(
                ([groupLabel, bars]) => (
          ...
          <div className="flex flex-col gap-4">
                        ...
                        <span className="text-muted-foreground shrink-0 text-xs tabular-nums">
                          {r.adaptiveness}/15 · weakest:{" "}
                          {
                            (itemAverages(stats.rows).find(
                              (a) => a.id === ITEM_IDS.reduce(
                                (weakest, id) =>
                                  ((r.items as Record<string, number>)[id] ??
                                    Infinity) <
                                  ((r.items as Record<string, number>)[weakest] ??
                                    Infinity)
                                    ? id
                                    : weakest,
                                ITEM_IDS[0],
                ...
                <ul className="space-y-1.5">
                  {stats.attentionList.map((r) => (
                    ...
                    >
                      <ProgramLink row={r} />
                      <span className="text-muted-foreground shrink-0 text-xs tabular-nums">
                        {r.adaptiveness}/15 · exposure {(r.exposure ?? 0).toFixed(0)}
                      </span>
                    </li>
                  ))}
                </ul>
            ...
            <Card className="border-red-200 dark:border-red-900/40">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">Preconditions not met</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-muted-foreground mb-3 text-xs">
                  ...
                  {stats.gates.map((r) => (
                    ...
                    >
                      ...
                      <span className="text-muted-foreground shrink-0 text-xs">
                        {[
                          r.gates?.G1 === "not-met" ? "disciplinary foundation" : null,
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              ...
              {stats.faculties.map((f) => (
                <button
                  key={f.name}
                  onClick={() =>
                    setParam("faculty", facultyFilter === f.name ? null : f.name)
                  }
                  ...
                  <tr>
                    {COLUMNS.map((col) => (
                      <th
                        ...
                        className={`${thClass()} ${col.numeric ? "tabular-nums" : ""}`}
                      >
                        <button
                          onClick={() => {
                            if (sortKey === col.key)
                          ...
                          className="hover:text-foreground inline-flex items-center gap-1 uppercase tracking-[0.14em]"
                        >
                          {col.label}
                    ...
                    >
                      ...
                      <td className="relative px-3 py-1.5 whitespace-nowrap tabular-nums">
                        ...
                        {r.atThreshold && (
                          <span
                            aria-hidden="true"
                            className="border-muted-foreground absolute right-1 top-1/2 hidden h-3 w-px border-l md:inline-block"
