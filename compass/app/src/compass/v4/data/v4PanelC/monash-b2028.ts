// GENERATED FILE — DO NOT EDIT.
// Source: dfva/source/rubricV4.ts + dfva/source/evidence/*.json (panelCv4 blocks)
// Regenerate: npm --prefix scripts run dfva:gen-v4
import type { V4PanelC } from "../v4Meta";

const record: V4PanelC = {
  "instrument": "4.2-draft",
  "adaptiveness": 14,
  "workplace": 5,
  "C1": {
    "score": 3,
    "rationale": "Level 3: Structured communication of complex disciplinary concepts and technical arguments to diverse stakeholders in communication and media studies.",
    "evidenceLines": [
      "evaluate the 'economic way of thinking' about a broad range of individual, business and government choices and decisions",
      "apply economic principles to evaluate economic institutions, economic decisions and performance",
      "This double degree in business and media communication will give you broad business knowledge, specialised communications skills and the ability to work across a number of industries."
    ]
  },
  "C2": {
    "score": 3,
    "rationale": "Level 3: Critical evaluation of methodological assumptions, disciplinary arguments, and professional evidence in communication and media studies.",
    "evidenceLines": [
      "recognise regulatory and political environments and how they impact on business.",
      "1 - Exercise: 20% (Hurdle: None)",
      "It's also great combination if you're wanting comprehensive knowledge in both fields, as well as an edge in the competitive world of media and business."
    ]
  },
  "C3": {
    "score": 2,
    "rationale": "Level 2: Disciplinary problem solving applying quantitative, computational, or analytical frameworks within communication and media studies.",
    "evidenceLines": [
      "Subject: Microeconomics (ECF1100)",
      "Subject: Macroeconomics (ECF1200)",
      "2 - Quiz / Test: 30% (Hurdle: None)"
    ]
  },
  "C4": {
    "score": 3,
    "rationale": "Level 3: Interdisciplinary integration connecting core principles, professional practices, and evolving industry standards in communication and media studies.",
    "evidenceLines": [
      "3 - Examination: 50% (Hurdle: None)",
      "appreciate the influence of macroeconomic conditions on decision making in government, business and international trade",
      "You'll develop core business skills that can be used in the media industry, and you will also establish a strategic and critical understanding of written, visual and audio communication - and how to apply these skills - in the field of business."
    ]
  },
  "C5": {
    "score": 3,
    "rationale": "Level 3: Applied investigation and empirical project work addressing real-world challenges in communication and media studies.",
    "evidenceLines": [
      "Subject: Economics (ECX2953)",
      "Subject: Microeconomics (ECF1100)",
      "understand and evaluate the importance of factors that determine aggregate output, unemployment, interest rates, inflation and the exchange rate"
    ]
  },
  "W1": {
    "score": 2,
    "rationale": "Level 2: Professional conduct, ethical practice, and regulatory compliance aligned with standards in communication and media studies.",
    "evidenceLines": [
      "analyse and critically evaluate policies designed to affect individual behaviour and aggregate outcomes.",
      "This double degree in business and media communication will give you broad business knowledge, specialised communications skills and the ability to work across a number of industries.",
      "It's also great combination if you're wanting comprehensive knowledge in both fields, as well as an edge in the competitive world of media and business."
    ]
  },
  "W2": {
    "score": 2,
    "rationale": "Level 2: Authentic tasks, applied case studies, and industry-aligned scenarios reflecting workplace practice in communication and media studies.",
    "evidenceLines": [
      "3 - Written: 15% (Hurdle: None)",
      "Subject: Economics (ECX2953)",
      "You'll develop core business skills that can be used in the media industry, and you will also establish a strategic and critical understanding of written, visual and audio communication - and how to apply these skills - in the field of business."
    ]
  },
  "W3": {
    "score": 1,
    "rationale": "Level 1: Academic coursework curriculum with elective industry engagements and experiential learning.",
    "evidenceLines": [
      "You'll develop core business skills that can be used in the media industry, and you will also establish a strategic and critical understanding of written, visual and audio communication - and how to apply these skills - in the field of business. By studying complementary subjects, you'll also build on your entrepreneurial expertise and gain practical skills to prepare you for the workplace.",
      "This double degree in business and media communication will give you broad business knowledge, specialised communications skills and the ability to work across a number of industries."
    ]
  },
  "gates": {
    "G1": {
      "result": "PASS",
      "rationale": "Postgraduate or undergraduate degree delivering comprehensive education in Bachelor of Business and Bachelor of Media Communication.",
      "evidenceLines": [
        "You'll develop core business skills that can be used in the media industry, and you will also establish a strategic and critical understanding of written, visual and audio communication - and how to apply these skills - in the field of business. By studying complementary subjects, you'll also build on your entrepreneurial expertise and gain practical skills to prepare you for the workplace.",
        "Refer to each partner course for the post nominals for the degree awarded"
      ]
    },
    "G2": {
      "result": "PASS",
      "rationale": "Specialist academic program structured through sequential study in communication and media studies.",
      "evidenceLines": [
        "Subject: Microeconomics (ECF1100)",
        "evaluate the 'economic way of thinking' about a broad range of individual, business and government choices and decisions"
      ]
    }
  },
  "ambiguities": [
    "C3: Scored at Level 2 for applied analytical problem solving without advanced algorithmic abstraction.",
    "W3: Scored at Level 1 as external placement is an elective rather than a universal requirement."
  ],
  "notScoreable": []
};

export default record;
