// GENERATED FILE — DO NOT EDIT.
// Source: dfva/source/rubricV4.ts + dfva/source/evidence/*.json (panelCv4 blocks)
// Regenerate: npm --prefix scripts run dfva:gen-v4
import type { V4PanelC } from "../v4Meta";

const record: V4PanelC = {
  "instrument": "4.2-draft",
  "adaptiveness": 12,
  "workplace": 5,
  "C1": {
    "score": 3,
    "rationale": "Level 3: Structured communication of complex disciplinary concepts and technical arguments to diverse stakeholders in business and management.",
    "evidenceLines": [
      "analyse change processes at an individual and organisational level",
      "demonstrate an understanding of the case for and challenges of planned and unplanned change to respond to changing organisational needs",
      "The Graduate Certificate of Human Resource management is a forward focused and evidence-based engagement with HR’s role in the emerging environment characterised by change and uncertainty."
    ]
  },
  "C2": {
    "score": 2,
    "rationale": "Level 2: Critical evaluation of methodological assumptions, disciplinary arguments, and professional evidence in business and management.",
    "evidenceLines": [
      "evaluate a range of frameworks, approaches, and research to guide the implementation of change strategies to develop a more efficient, effective, and cohesive workplace.",
      "demonstrate knowledge or skills for crafting sustainable futures for people, organisations, communities, and/or the environment.",
      "With a focus on ‘the future of work’ and ‘the future of HRM’ you will learn about current views regarding the changing nature of HRM brought about by changes to the organisational and employment context."
    ]
  },
  "C3": {
    "score": 2,
    "rationale": "Level 2: Disciplinary problem solving applying quantitative, computational, or analytical frameworks within business and management.",
    "evidenceLines": [
      "Subject: People and change (MGO5100)",
      "Subject: Sustainability and performance in human resource management (MGO5110)",
      "1 - Written: 40% (Hurdle: None)"
    ]
  },
  "C4": {
    "score": 3,
    "rationale": "Level 3: Interdisciplinary integration connecting core principles, professional practices, and evolving industry standards in business and management.",
    "evidenceLines": [
      "2 - Written: 60% (Hurdle: None)",
      "demonstrate an understanding of workforce sustainability and performance challenges and opportunities for organisations",
      "This includes contemporary challenges such as COVID and other major disruptions now and in the future, technological disruption, sustainability (of work, organisations and the environment) and an emerging challenge of the ‘human’ element of organisational success."
    ]
  },
  "C5": {
    "score": 2,
    "rationale": "Level 2: Applied investigation and empirical project work addressing real-world challenges in business and management.",
    "evidenceLines": [
      "Subject: Ethics and legal issues in human resource management (MGO5120)",
      "Subject: Diversity and inclusion  (MGO5130)",
      "demonstrate knowledge or skills for crafting sustainable futures for people, organisations, communities, and/or the environment."
    ]
  },
  "W1": {
    "score": 2,
    "rationale": "Level 2: Professional conduct, ethical practice, and regulatory compliance aligned with standards in business and management.",
    "evidenceLines": [
      "critically analyse and evaluate HRM’s contribution to organisational performance and workforce sustainability by enhancing competitive advantage, employee wellbeing, and societal outcomes in times of uncertainty and change",
      "The Graduate Certificate of Human Resource management is a forward focused and evidence-based engagement with HR’s role in the emerging environment characterised by change and uncertainty.",
      "With a focus on ‘the future of work’ and ‘the future of HRM’ you will learn about current views regarding the changing nature of HRM brought about by changes to the organisational and employment context."
    ]
  },
  "W2": {
    "score": 2,
    "rationale": "Level 2: Authentic tasks, applied case studies, and industry-aligned scenarios reflecting workplace practice in business and management.",
    "evidenceLines": [
      "develop the knowledge and skills to improve HRM’s contribution to the sustainability of the workforce and organisational performance.",
      "Subject: Diversity and inclusion  (MGO5130)",
      "This includes contemporary challenges such as COVID and other major disruptions now and in the future, technological disruption, sustainability (of work, organisations and the environment) and an emerging challenge of the ‘human’ element of organisational success."
    ]
  },
  "W3": {
    "score": 1,
    "rationale": "Level 1: Academic coursework curriculum with elective industry engagements and experiential learning.",
    "evidenceLines": [
      "The course comprises 24 credit points.",
      "The Graduate Certificate of Human Resource management is a forward focused and evidence-based engagement with HR’s role in the emerging environment characterised by change and uncertainty."
    ]
  },
  "gates": {
    "G1": {
      "result": "PASS",
      "rationale": "Postgraduate or undergraduate degree delivering comprehensive education in Graduate Certificate of Human Resource Management.",
      "evidenceLines": [
        "The course comprises 24 credit points.",
        "You must complete the following four units (24 credit points)"
      ]
    },
    "G2": {
      "result": "PASS",
      "rationale": "Specialist academic program structured through sequential study in business and management.",
      "evidenceLines": [
        "Subject: People and change (MGO5100)",
        "analyse change processes at an individual and organisational level"
      ]
    }
  },
  "ambiguities": [
    "C3: Scored at Level 2 for applied analytical problem solving without advanced algorithmic abstraction.",
    "W3: Scored at Level 1 as external placement is an elective rather than a universal requirement."
  ],
  "notScoreable": []
};

export default record;
