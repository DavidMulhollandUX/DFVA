// GENERATED FILE — DO NOT EDIT.
// Source: dfva/source/rubricV4.ts + dfva/source/evidence/*.json (panelCv4 blocks)
// Regenerate: npm --prefix scripts run dfva:gen-v4
import type { V4PanelC } from "../v4Meta";

const record: V4PanelC = {
  "instrument": "4.2-draft",
  "adaptiveness": 13,
  "workplace": 9,
  "C1": {
    "score": 3,
    "rationale": "Level 3: Structured communication of complex disciplinary concepts and technical arguments to diverse stakeholders in health, n.e.c..",
    "evidenceLines": [
      "**Institution**: The University of Sydney (USYD)",
      "**Course / Program Identifier**: oral-health",
      "- **Institution**: The University of Sydney (USYD)"
    ]
  },
  "C2": {
    "score": 3,
    "rationale": "Level 3: Critical evaluation of methodological assumptions, disciplinary arguments, and professional evidence in health, n.e.c..",
    "evidenceLines": [
      "**Academic Year**: 2026",
      "**Faculty**: medicine-health",
      "- **Course / Program Identifier**: oral-health"
    ]
  },
  "C3": {
    "score": 2,
    "rationale": "Level 2: Disciplinary problem solving applying quantitative, computational, or analytical frameworks within health, n.e.c..",
    "evidenceLines": [
      "Subject: Foundations of Oral Health I (ORHL1101)",
      "Subject: Oral Health Clinical Practice I (ORHL1102)",
      "**Handbook URL**: https://www.sydney.edu.au/handbooks/medicine-health/coursework/oral-health.html"
    ]
  },
  "C4": {
    "score": 3,
    "rationale": "Level 3: Interdisciplinary integration connecting core principles, professional practices, and evolving industry standards in health, n.e.c..",
    "evidenceLines": [
      "LO1 . Describe the hard and soft tissues of the oral anatomy relevant to the development and treatment of oral disease, and administration of local anaesthesia",
      "LO2 . Describe the development of oral disease, the associated host response, and preventive measures",
      "- **Handbook URL**: https://www.sydney.edu.au/handbooks/medicine-health/coursework/oral-health.html"
    ]
  },
  "C5": {
    "score": 2,
    "rationale": "Level 2: Applied investigation and empirical project work addressing real-world challenges in health, n.e.c..",
    "evidenceLines": [
      "Subject: Life Sciences I (ORHL1104)",
      "Subject: Oral Health Clinical Practice II (ORHL1202)",
      "LO3 . Describe individual patient risk factors for oral disease and appropriate management strategies"
    ]
  },
  "W1": {
    "score": 3,
    "rationale": "Level 3: Professional conduct, ethical practice, and regulatory compliance aligned with standards in health, n.e.c..",
    "evidenceLines": [
      "LO4 . Identify and interpret strategies for successful transition to University and the Oral Health profession",
      "- **Institution**: The University of Sydney (USYD)",
      "- **Course / Program Identifier**: oral-health"
    ]
  },
  "W2": {
    "score": 3,
    "rationale": "Level 3: Authentic tasks, applied case studies, and industry-aligned scenarios reflecting workplace practice in health, n.e.c..",
    "evidenceLines": [
      "LO5 . Discuss the general social, cultural, environmental and economic factors which impact individual and community health",
      "Subject: Life Sciences III (ORHL2104)",
      "- **Handbook URL**: https://www.sydney.edu.au/handbooks/medicine-health/coursework/oral-health.html"
    ]
  },
  "W3": {
    "score": 3,
    "rationale": "Level 3: Academic coursework curriculum with compulsory work-integrated learning or clinical placement.",
    "evidenceLines": [
      "(i) a fixed curriculum of 144 credit points in the order prescribed in the following table",
      "- **Institution**: The University of Sydney (USYD)"
    ]
  },
  "gates": {
    "G1": {
      "result": "PASS",
      "rationale": "Postgraduate or undergraduate degree delivering comprehensive education in Oral Health.",
      "evidenceLines": [
        "(i) a fixed curriculum of 144 credit points in the order prescribed in the following table",
        "C: Corequisites N: Prohibition"
      ]
    },
    "G2": {
      "result": "PASS",
      "rationale": "Specialist academic program structured through sequential study in health, n.e.c..",
      "evidenceLines": [
        "Subject: Foundations of Oral Health I (ORHL1101)",
        "**Institution**: The University of Sydney (USYD)"
      ]
    }
  },
  "ambiguities": [
    "C3: Scored at Level 2 for applied analytical problem solving without advanced algorithmic abstraction.",
    "W3: Scored at Level 2 or 3 due to compulsory embedded workplace or clinical practicum."
  ],
  "notScoreable": []
};

export default record;
