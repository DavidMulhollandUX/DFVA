// GENERATED FILE — DO NOT EDIT.
// Source: dfva/source/rubricV4.ts + dfva/source/evidence/*.json (panelCv4 blocks)
// Regenerate: npm --prefix scripts run dfva:gen-v4
import type { V4PanelC } from "../v4Meta";

const record: V4PanelC = {
  "instrument": "4.2-draft",
  "adaptiveness": 13,
  "workplace": 9,
  "C1": {
    "score": 3,
    "rationale": "Level 3: Structured communication of complex disciplinary concepts and technical arguments to diverse stakeholders in general nursing.",
    "evidenceLines": [
      "**Institution**: The University of Sydney (USYD)",
      "**Course / Program Identifier**: arts-nursing",
      "- **Institution**: The University of Sydney (USYD)"
    ]
  },
  "C2": {
    "score": 3,
    "rationale": "Level 3: Critical evaluation of methodological assumptions, disciplinary arguments, and professional evidence in general nursing.",
    "evidenceLines": [
      "**Academic Year**: 2026",
      "**Handbook URL**: https://www.sydney.edu.au/handbooks/arts/coursework/arts-nursing.html",
      "- **Course / Program Identifier**: arts-nursing"
    ]
  },
  "C3": {
    "score": 2,
    "rationale": "Level 2: Disciplinary problem solving applying quantitative, computational, or analytical frameworks within general nursing.",
    "evidenceLines": [
      "Subject: NURS5201 - Fundamentals of Human Pathophysiology",
      "Subject: NURS5202 - Foundations of Nursing Practice",
      "LO1 . Describe basic human anatomy and physiological function within the body"
    ]
  },
  "C4": {
    "score": 3,
    "rationale": "Level 3: Interdisciplinary integration connecting core principles, professional practices, and evolving industry standards in general nursing.",
    "evidenceLines": [
      "LO2 . Describe the pathophysiology of selected health priority areas to explain their respective clinical manifestations, prevention, and pharmacological management.",
      "LO3 . Synthetise, integrate and apply foundational human bioscience knowledge to specific contexts and situations in health",
      "- **Handbook URL**: https://www.sydney.edu.au/handbooks/arts/coursework/arts-nursing.html"
    ]
  },
  "C5": {
    "score": 2,
    "rationale": "Level 2: Applied investigation and empirical project work addressing real-world challenges in general nursing.",
    "evidenceLines": [
      "Subject: NURS5204 - Illness Experiences and Nursing Practice",
      "Subject: NURS5205 - Pharmacotherapeutics for Nursing",
      "LO4 . Describe how the structure of cells and tissues are related to their roles within selected organs and organ systems."
    ]
  },
  "W1": {
    "score": 3,
    "rationale": "Level 3: Professional conduct, ethical practice, and regulatory compliance aligned with standards in general nursing.",
    "evidenceLines": [
      "LO5 . Explain pathogenic processes within the human body and their expression as observable alterations to normal anatomy/ physiological function across the lifespan",
      "- **Institution**: The University of Sydney (USYD)",
      "- **Course / Program Identifier**: arts-nursing"
    ]
  },
  "W2": {
    "score": 3,
    "rationale": "Level 3: Authentic tasks, applied case studies, and industry-aligned scenarios reflecting workplace practice in general nursing.",
    "evidenceLines": [
      "Subject: NURS5002 - Social Contexts of Health",
      "LO6 . Describe how specific organ systems contribute to the maintenance of homeostasis and thereby support overall health.",
      "- **Faculty**: arts"
    ]
  },
  "W3": {
    "score": 3,
    "rationale": "Level 3: Academic coursework curriculum with compulsory work-integrated learning or clinical placement.",
    "evidenceLines": [
      "Undergraduate coursework degrees",
      "Arts and Social Sciences Undergraduate",
      "Subject: NURS5202 - Foundations of Nursing Practice"
    ]
  },
  "gates": {
    "G1": {
      "result": "PASS",
      "rationale": "Postgraduate or undergraduate degree delivering comprehensive education in Bachelor of Arts and Master of Nursing.",
      "evidenceLines": [
        "- **Institution**: The University of Sydney (USYD)",
        "- **Faculty**: arts"
      ]
    },
    "G2": {
      "result": "PASS",
      "rationale": "Specialist academic program structured through sequential study in general nursing.",
      "evidenceLines": [
        "- **Course / Program Identifier**: arts-nursing",
        "Subject: NURS5201 - Fundamentals of Human Pathophysiology"
      ]
    }
  },
  "ambiguities": [
    "C3: Scored at Level 2 for applied analytical problem solving without advanced algorithmic abstraction.",
    "W3: Scored at Level 2 or 3 due to compulsory embedded workplace or clinical practicum."
  ],
  "notScoreable": []
};

export default record;
