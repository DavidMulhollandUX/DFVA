// GENERATED FILE — DO NOT EDIT.
// Source: dfva/source/rubricV4.ts + dfva/source/evidence/*.json (panelCv4 blocks)
// Regenerate: npm --prefix scripts run dfva:gen-v4
import type { V4PanelC } from "../v4Meta";

const record: V4PanelC = {
  "instrument": "4.2-draft",
  "adaptiveness": 12,
  "workplace": 5,
  "C1": {
    "score": 3,
    "rationale": "Level 3: Structured communication of complex disciplinary concepts and technical arguments to diverse stakeholders in business and management.",
    "evidenceLines": [
      "you have achieved a minimum weighted average mark of 80% in the units of this course;",
      "you have the permission of the Course Director; and",
      "Sustainability represents an increasingly influential facet of tourism planning, development, governance and management, encompassing a diversity of sectors as the industry continues to expand globally."
    ]
  },
  "C2": {
    "score": 2,
    "rationale": "Level 2: Critical evaluation of methodological assumptions, disciplinary arguments, and professional evidence in business and management.",
    "evidenceLines": [
      "a suitable thesis supervisor is available.",
      "APG5084 Designing research as offered in Part C.",
      "The United Nations World Tourism Organization (UNWTO) is also forecasting that the growth rate for emerging economies will be double the rate of advanced economies in the years leading to 2030 and that a sustainable industry can therefore play a significant role in the achievement of development goals."
    ]
  },
  "C3": {
    "score": 2,
    "rationale": "Level 2: Disciplinary problem solving applying quantitative, computational, or analytical frameworks within business and management.",
    "evidenceLines": [
      "Subject: Professional internship (APG5044)",
      "Subject: Sustainable cultural development (APG5067)",
      "APG5849 Research thesis A replaces 6 credit points from Part D."
    ]
  },
  "C4": {
    "score": 3,
    "rationale": "Level 3: Interdisciplinary integration connecting core principles, professional practices, and evolving industry standards in business and management.",
    "evidenceLines": [
      "APG5850 Research thesis B replaces 12 credit points from Part B.",
      "you have achieved a minimum weighted average mark of 80% in the units of this course;",
      "Effective planning and policy making for international tourism, and for the management of environmental, socio-cultural and economic consequences is critical."
    ]
  },
  "C5": {
    "score": 2,
    "rationale": "Level 2: Applied investigation and empirical project work addressing real-world challenges in business and management.",
    "evidenceLines": [
      "Subject: Creative cities (APG5068)",
      "Subject: Designing research (APG5084)",
      "you have the permission of the Course Director; and"
    ]
  },
  "W1": {
    "score": 2,
    "rationale": "Level 2: Professional conduct, ethical practice, and regulatory compliance aligned with standards in business and management.",
    "evidenceLines": [
      "a suitable thesis supervisor is available.",
      "Sustainability represents an increasingly influential facet of tourism planning, development, governance and management, encompassing a diversity of sectors as the industry continues to expand globally.",
      "The United Nations World Tourism Organization (UNWTO) is also forecasting that the growth rate for emerging economies will be double the rate of advanced economies in the years leading to 2030 and that a sustainable industry can therefore play a significant role in the achievement of development goals."
    ]
  },
  "W2": {
    "score": 2,
    "rationale": "Level 2: Authentic tasks, applied case studies, and industry-aligned scenarios reflecting workplace practice in business and management.",
    "evidenceLines": [
      "Entry level 1: 30 credit points from the elective lists below.",
      "Subject: Cultural tourism and events management (APG5128)",
      "Effective planning and policy making for international tourism, and for the management of environmental, socio-cultural and economic consequences is critical."
    ]
  },
  "W3": {
    "score": 1,
    "rationale": "Level 1: Academic coursework curriculum with elective industry engagements and experiential learning.",
    "evidenceLines": [
      "The course is structured in four parts. Part A. Core studies, Part B. Applied studies, Part C. Professional practice studies and Part D. Elective studies.",
      "Sustainability represents an increasingly influential facet of tourism planning, development, governance and management, encompassing a diversity of sectors as the industry continues to expand globally."
    ]
  },
  "gates": {
    "G1": {
      "result": "PASS",
      "rationale": "Postgraduate or undergraduate degree delivering comprehensive education in Master of International Sustainable Tourism Management.",
      "evidenceLines": [
        "The course is structured in four parts. Part A. Core studies, Part B. Applied studies, Part C. Professional practice studies and Part D. Elective studies.",
        "Part A. Core studies"
      ]
    },
    "G2": {
      "result": "PASS",
      "rationale": "Specialist academic program structured through sequential study in business and management.",
      "evidenceLines": [
        "Subject: Professional internship (APG5044)",
        "you have achieved a minimum weighted average mark of 80% in the units of this course;"
      ]
    }
  },
  "ambiguities": [
    "C3: Scored at Level 2 for applied analytical problem solving without advanced algorithmic abstraction.",
    "W3: Scored at Level 1 as external placement is an elective rather than a universal requirement."
  ],
  "notScoreable": []
};

export default record;
