// GENERATED FILE — DO NOT EDIT.
// Source: dfva/source/rubricV4.ts + dfva/source/evidence/*.json (panelCv4 blocks)
// Regenerate: npm --prefix scripts run dfva:gen-v4
import type { V4PanelC } from "../v4Meta";

const record: V4PanelC = {
  "instrument": "4.2-draft",
  "adaptiveness": 15,
  "workplace": 6,
  "C1": {
    "score": 3,
    "rationale": "Level 3: Structured communication of complex disciplinary concepts and technical arguments to diverse stakeholders in computer science.",
    "evidenceLines": [
      "Reflect critically on your own cultural attitudes, values and beliefs",
      "Demonstrate knowledge of and respect for Indigenous Australian perspectives and experiences, history, culture and customs, including an appreciation of the local people and country",
      "La Trobe's Undergraduate Certificate in Information Technolgy provides you with the foundational knowledge to enhance your employability with a digital skillset or pursue a new career in information technology."
    ]
  },
  "C2": {
    "score": 3,
    "rationale": "Level 3: Critical evaluation of methodological assumptions, disciplinary arguments, and professional evidence in computer science.",
    "evidenceLines": [
      "Recall important details regarding the history, culture, customs and people of Indigenous Australia",
      "Understand the need for and value of appropriate and effective intercultural communication",
      "You'll build solid foundations and essential knowledge across key areas of information technology in like networking, programming, databases and more."
    ]
  },
  "C3": {
    "score": 3,
    "rationale": "Level 3: Disciplinary problem solving applying quantitative, computational, or analytical frameworks within computer science.",
    "evidenceLines": [
      "Subject: WOMINJEKA LA TROBE: INDIGENOUS CULTURAL LITERACY FOR HIGHER EDUCATION (ABS0WOM)",
      "Subject: INTRODUCTION TO CYBERSECURITY (CSE1ICB)",
      "Apply and compare approaches to cyber risk management in addressing real-world digital security challenges."
    ]
  },
  "C4": {
    "score": 3,
    "rationale": "Level 3: Interdisciplinary integration connecting core principles, professional practices, and evolving industry standards in computer science.",
    "evidenceLines": [
      "Demonstrate foundation skills in identifying malware, securing applications, systems, and networks",
      "Analyse emerging cybersecurity practices, regulations, and standards, and evaluate their implications for digital security.",
      "You'll develop the foundational knowledge in the areas of computer hardware, cybersecurity, programming, software, data communications, and computer networks."
    ]
  },
  "C5": {
    "score": 3,
    "rationale": "Level 3: Applied investigation and empirical project work addressing real-world challenges in computer science.",
    "evidenceLines": [
      "Subject: DATABASE FUNDAMENTALS (CSE2DBF)",
      "Subject: NETWORK ENGINEERING FUNDAMENTALS (CSE2NEF)",
      "Work efficiently and effectively as a member of a team to investigate possible solutions such as cryptography to enhance data security, and web security, and digital forensics against cyber threats"
    ]
  },
  "W1": {
    "score": 2,
    "rationale": "Level 2: Professional conduct, ethical practice, and regulatory compliance aligned with standards in computer science.",
    "evidenceLines": [
      "Identify the appropriate normalization techniques for database design.",
      "La Trobe's Undergraduate Certificate in Information Technolgy provides you with the foundational knowledge to enhance your employability with a digital skillset or pursue a new career in information technology.",
      "You'll build solid foundations and essential knowledge across key areas of information technology in like networking, programming, databases and more."
    ]
  },
  "W2": {
    "score": 3,
    "rationale": "Level 3: Authentic tasks, applied case studies, and industry-aligned scenarios reflecting workplace practice in computer science.",
    "evidenceLines": [
      "Identify the possible risks and ethical and social considerations relevant to a designed system.",
      "Subject: ACADEMIC INTEGRITY MODULE (LTU0AIM)",
      "You'll develop the foundational knowledge in the areas of computer hardware, cybersecurity, programming, software, data communications, and computer networks."
    ]
  },
  "W3": {
    "score": 1,
    "rationale": "Level 1: Academic coursework curriculum with elective industry engagements and experiential learning.",
    "evidenceLines": [
      "* Protecting data, systems and networks  * Understand the core concepts, terminology, and practices of cybersecurity, exploring common attacks, malware, real-world data breaches, and apply basic security controls and risk management approaches.",
      "La Trobe's Undergraduate Certificate in Information Technolgy provides you with the foundational knowledge to enhance your employability with a digital skillset or pursue a new career in information technology."
    ]
  },
  "gates": {
    "G1": {
      "result": "PASS",
      "rationale": "Postgraduate or undergraduate degree delivering comprehensive education in Undergraduate Certificate in Information Technology.",
      "evidenceLines": [
        "* Protecting data, systems and networks  * Understand the core concepts, terminology, and practices of cybersecurity, exploring common attacks, malware, real-world data breaches, and apply basic security controls and risk management approaches.",
        "60 core credit points"
      ]
    },
    "G2": {
      "result": "PASS",
      "rationale": "Specialist academic program structured through sequential study in computer science.",
      "evidenceLines": [
        "Subject: WOMINJEKA LA TROBE: INDIGENOUS CULTURAL LITERACY FOR HIGHER EDUCATION (ABS0WOM)",
        "Reflect critically on your own cultural attitudes, values and beliefs"
      ]
    }
  },
  "ambiguities": [
    "C3: Scored at Level 3 due to advanced analytical, mathematical, or computational curriculum content.",
    "W3: Scored at Level 1 as external placement is an elective rather than a universal requirement."
  ],
  "notScoreable": []
};

export default record;
