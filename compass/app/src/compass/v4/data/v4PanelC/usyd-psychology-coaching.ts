// GENERATED FILE — DO NOT EDIT.
// Source: dfva/source/rubricV4.ts + dfva/source/evidence/*.json (panelCv4 blocks)
// Regenerate: npm --prefix scripts run dfva:gen-v4
import type { V4PanelC } from "../v4Meta";

const record: V4PanelC = {
  "instrument": "4.2-draft",
  "adaptiveness": 12,
  "workplace": 5,
  "C1": {
    "score": 3,
    "rationale": "Level 3: Structured communication of complex disciplinary concepts and technical arguments to diverse stakeholders in psychology.",
    "evidenceLines": [
      "**Institution**: The University of Sydney (USYD)",
      "**Course / Program Identifier**: psychology-coaching",
      "- **Institution**: The University of Sydney (USYD)"
    ]
  },
  "C2": {
    "score": 2,
    "rationale": "Level 2: Critical evaluation of methodological assumptions, disciplinary arguments, and professional evidence in psychology.",
    "evidenceLines": [
      "**Academic Year**: 2026",
      "**Faculty**: science-pg",
      "- **Course / Program Identifier**: psychology-coaching"
    ]
  },
  "C3": {
    "score": 2,
    "rationale": "Level 2: Disciplinary problem solving applying quantitative, computational, or analytical frameworks within psychology.",
    "evidenceLines": [
      "Subject: Coaching Practice (PSYC4724)",
      "Subject: Groups, Teams and Systems (PSYC4729)",
      "**Handbook URL**: https://www.sydney.edu.au/handbooks/science-pg/coursework/psychology-coaching.html"
    ]
  },
  "C4": {
    "score": 3,
    "rationale": "Level 3: Interdisciplinary integration connecting core principles, professional practices, and evolving industry standards in psychology.",
    "evidenceLines": [
      "LO1 . develop understanding and competency in coaching practice and its microskills",
      "LO2 . develop understanding and competency in case conceptualisation",
      "- **Handbook URL**: https://www.sydney.edu.au/handbooks/science-pg/coursework/psychology-coaching.html"
    ]
  },
  "C5": {
    "score": 2,
    "rationale": "Level 2: Applied investigation and empirical project work addressing real-world challenges in psychology.",
    "evidenceLines": [
      "Subject: Psychology of Peak Performance (PSYC4731)",
      "Subject: Research Project in Applied Psychology A (PSYC5016)",
      "LO3 . understand personality styles and disorders and appropriate coaching responses"
    ]
  },
  "W1": {
    "score": 2,
    "rationale": "Level 2: Professional conduct, ethical practice, and regulatory compliance aligned with standards in psychology.",
    "evidenceLines": [
      "LO4 . to understand the theory and application of defence mechanisms, transference and counter-transference in coaching",
      "- **Institution**: The University of Sydney (USYD)",
      "- **Course / Program Identifier**: psychology-coaching"
    ]
  },
  "W2": {
    "score": 2,
    "rationale": "Level 2: Authentic tasks, applied case studies, and industry-aligned scenarios reflecting workplace practice in psychology.",
    "evidenceLines": [
      "LO5 . To understand and explore the application of supervision theories in coaching",
      "Subject: Research Project in Applied Psychology A (PSYC5016)",
      "- **Handbook URL**: https://www.sydney.edu.au/handbooks/science-pg/coursework/psychology-coaching.html"
    ]
  },
  "W3": {
    "score": 1,
    "rationale": "Level 1: Academic coursework curriculum with elective industry engagements and experiential learning.",
    "evidenceLines": [
      "The Coaching Psychology postgraduate requirements are detailed in the",
      "- **Institution**: The University of Sydney (USYD)"
    ]
  },
  "gates": {
    "G1": {
      "result": "PASS",
      "rationale": "Postgraduate or undergraduate degree delivering comprehensive education in Coaching Psychology.",
      "evidenceLines": [
        "The Coaching Psychology postgraduate requirements are detailed in the",
        "Students in the coursework pathway must complete 48 credit points, including:"
      ]
    },
    "G2": {
      "result": "PASS",
      "rationale": "Specialist academic program structured through sequential study in psychology.",
      "evidenceLines": [
        "Subject: Coaching Practice (PSYC4724)",
        "**Institution**: The University of Sydney (USYD)"
      ]
    }
  },
  "ambiguities": [
    "C3: Scored at Level 2 for applied analytical problem solving without advanced algorithmic abstraction.",
    "W3: Scored at Level 1 as external placement is an elective rather than a universal requirement."
  ],
  "notScoreable": []
};

export default record;
