// GENERATED FILE — DO NOT EDIT.
// Source: dfva/source/rubricV4.ts + dfva/source/evidence/*.json (panelCv4 blocks)
// Regenerate: npm --prefix scripts run dfva:gen-v4
import type { V4PanelC } from "../v4Meta";

const record: V4PanelC = {
  "instrument": "4.2-draft",
  "adaptiveness": 12,
  "workplace": 5,
  "C1": {
    "score": 3,
    "rationale": "Level 3: Structured communication of complex disciplinary concepts and technical arguments to diverse stakeholders in management and commerce.",
    "evidenceLines": [
      "**Institution**: The University of Adelaide",
      "**Program Identifier / Slug**: mtil_mtchinld",
      "- **Institution**: The University of Adelaide"
    ]
  },
  "C2": {
    "score": 2,
    "rationale": "Level 2: Critical evaluation of methodological assumptions, disciplinary arguments, and professional evidence in management and commerce.",
    "evidenceLines": [
      "**Academic Calendar Year**: 2025/2026",
      "**Faculty**: Faculty of Arts, Business, Law and Economics (ABLE)",
      "- **Program Identifier / Slug**: mtil_mtchinld"
    ]
  },
  "C3": {
    "score": 2,
    "rationale": "Level 2: Disciplinary problem solving applying quantitative, computational, or analytical frameworks within management and commerce.",
    "evidenceLines": [
      "Subject: Leading New Product Innovation (BUSI5029)",
      "Subject: Navigating Commercialisation Pathways (BUSI5004)",
      "**Handbook / Calendar URL**: https://calendar.adelaide.edu.au/aprcw/2025/mtil_mtchinld"
    ]
  },
  "C4": {
    "score": 3,
    "rationale": "Level 3: Interdisciplinary integration connecting core principles, professional practices, and evolving industry standards in management and commerce.",
    "evidenceLines": [
      "Summarise, justify and explain key underlying factors that form an effective product innovation process",
      "Develop and lead innovation that will consider ethical, social and environmental impact, culture and diversity in the creation and progressive development of new product innovations",
      "- **Academic Calendar Year**: 2025/2026"
    ]
  },
  "C5": {
    "score": 2,
    "rationale": "Level 2: Applied investigation and empirical project work addressing real-world challenges in management and commerce.",
    "evidenceLines": [
      "Subject: Strategic Innovation Management (BUSI5032)",
      "Subject: Entrepreneurial Business Development (BUSI5031)",
      "Critically analyse and evaluate new product ideas in order to prioritise potential to address key innovation lenses and technology requirements"
    ]
  },
  "W1": {
    "score": 2,
    "rationale": "Level 2: Professional conduct, ethical practice, and regulatory compliance aligned with standards in management and commerce.",
    "evidenceLines": [
      "Create and communicate a value proposition to solve a market problem",
      "- **Institution**: The University of Adelaide",
      "- **Program Identifier / Slug**: mtil_mtchinld"
    ]
  },
  "W2": {
    "score": 2,
    "rationale": "Level 2: Authentic tasks, applied case studies, and industry-aligned scenarios reflecting workplace practice in management and commerce.",
    "evidenceLines": [
      "Persuasive Piece: 20%% (Hurdle: False)",
      "Subject: Business Research Methods (M) (BUSI5124)",
      "- **Academic Calendar Year**: 2025/2026"
    ]
  },
  "W3": {
    "score": 1,
    "rationale": "Level 1: Academic coursework curriculum with elective industry engagements and experiential learning.",
    "evidenceLines": [
      "The Master of Technology Innovation and Leadership is an AQF Level 9 Masters qualification with a standard full-time duration of 2 years. Students may be eligible to complete the program in a full-time standard duration of 1.5 years on the basis of prior learning.",
      "- **Institution**: The University of Adelaide"
    ]
  },
  "gates": {
    "G1": {
      "result": "PASS",
      "rationale": "Postgraduate or undergraduate degree delivering comprehensive education in Master of Technology Innovation and Leadership.",
      "evidenceLines": [
        "The Master of Technology Innovation and Leadership is an AQF Level 9 Masters qualification with a standard full-time duration of 2 years. Students may be eligible to complete the program in a full-time standard duration of 1.5 years on the basis of prior learning.",
        "Program Maximum Duration"
      ]
    },
    "G2": {
      "result": "PASS",
      "rationale": "Specialist academic program structured through sequential study in management and commerce.",
      "evidenceLines": [
        "Subject: Leading New Product Innovation (BUSI5029)",
        "**Institution**: The University of Adelaide"
      ]
    }
  },
  "ambiguities": [
    "C3: Scored at Level 2 for applied analytical problem solving without advanced algorithmic abstraction.",
    "W3: Scored at Level 1 as external placement is an elective rather than a universal requirement."
  ],
  "notScoreable": []
};

export default record;
