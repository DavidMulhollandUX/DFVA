// GENERATED FILE — DO NOT EDIT.
// Source: dfva/source/rubricV4.ts + dfva/source/evidence/*.json (panelCv4 blocks)
// Regenerate: npm --prefix scripts run dfva:gen-v4
import type { V4PanelC } from "../v4Meta";

const record: V4PanelC = {
  "instrument": "4.2-draft",
  "adaptiveness": 12,
  "workplace": 9,
  "C1": {
    "score": 3,
    "rationale": "Level 3: Structured communication of complex disciplinary concepts and technical arguments to diverse stakeholders in teacher education.",
    "evidenceLines": [
      "**Institution**: The University of Sydney (USYD)",
      "**Course / Program Identifier**: education-early-childhood",
      "- **Institution**: The University of Sydney (USYD)"
    ]
  },
  "C2": {
    "score": 2,
    "rationale": "Level 2: Critical evaluation of methodological assumptions, disciplinary arguments, and professional evidence in teacher education.",
    "evidenceLines": [
      "**Academic Year**: 2026",
      "**Handbook URL**: https://www.sydney.edu.au/handbooks/arts/coursework/education-early-childhood.html",
      "- **Course / Program Identifier**: education-early-childhood"
    ]
  },
  "C3": {
    "score": 2,
    "rationale": "Level 2: Disciplinary problem solving applying quantitative, computational, or analytical frameworks within teacher education.",
    "evidenceLines": [
      "Subject: Introduction to EC Education (EDEC1005)",
      "Subject: Learning Through Play in Early Childhood (EDEC1006)",
      "Critically evaluate foundational principles, evidence bases, and contemporary methodologies in introduction to ec education."
    ]
  },
  "C4": {
    "score": 3,
    "rationale": "Level 3: Interdisciplinary integration connecting core principles, professional practices, and evolving industry standards in teacher education.",
    "evidenceLines": [
      "Synthesise disciplinary knowledge to resolve complex real-world challenges.",
      "Communicate evidence-based findings effectively to technical and professional stakeholders.",
      "- **Handbook URL**: https://www.sydney.edu.au/handbooks/arts/coursework/education-early-childhood.html"
    ]
  },
  "C5": {
    "score": 2,
    "rationale": "Level 2: Applied investigation and empirical project work addressing real-world challenges in teacher education.",
    "evidenceLines": [
      "Subject: Health and Wellbeing in Early Childhood (EDEC1007)",
      "Subject: History and Philosophy of ECE (EDEC1008)",
      "Apply ethical frameworks and professional standards in practice."
    ]
  },
  "W1": {
    "score": 3,
    "rationale": "Level 3: Professional conduct, ethical practice, and regulatory compliance aligned with standards in teacher education.",
    "evidenceLines": [
      "Critically evaluate foundational principles, evidence bases, and contemporary methodologies in learning through play in early childhood.",
      "- **Institution**: The University of Sydney (USYD)",
      "- **Course / Program Identifier**: education-early-childhood"
    ]
  },
  "W2": {
    "score": 3,
    "rationale": "Level 3: Authentic tasks, applied case studies, and industry-aligned scenarios reflecting workplace practice in teacher education.",
    "evidenceLines": [
      "Synthesise disciplinary knowledge to resolve complex real-world challenges.",
      "Subject: Creative Arts in Early Childhood 1 (EDEC1009)",
      "- **Handbook URL**: https://www.sydney.edu.au/handbooks/arts/coursework/education-early-childhood.html"
    ]
  },
  "W3": {
    "score": 3,
    "rationale": "Level 3: Academic coursework curriculum with compulsory work-integrated learning or clinical placement.",
    "evidenceLines": [
      "The Bachelor of Education (Early Childhood) is a four-year course comprised of specialist units in early childhood education and development, and complemented by generalist units in education and professional studies. Students complete 90 days of professional experience in up to four different early childhood settings. The degree has a strong emphasis on social justice and leadership in early childhood contexts.",
      "- **Institution**: The University of Sydney (USYD)"
    ]
  },
  "gates": {
    "G1": {
      "result": "PASS",
      "rationale": "Postgraduate or undergraduate degree delivering comprehensive education in Bachelor of Education.",
      "evidenceLines": [
        "The Bachelor of Education (Early Childhood) is a four-year course comprised of specialist units in early childhood education and development, and complemented by generalist units in education and professional studies. Students complete 90 days of professional experience in up to four different early childhood settings. The degree has a strong emphasis on social justice and leadership in early childhood contexts.",
        "Candidates must complete 192 credit points of units of study comprising:"
      ]
    },
    "G2": {
      "result": "PASS",
      "rationale": "Specialist academic program structured through sequential study in teacher education.",
      "evidenceLines": [
        "Subject: Introduction to EC Education (EDEC1005)",
        "**Institution**: The University of Sydney (USYD)"
      ]
    }
  },
  "ambiguities": [
    "C3: Scored at Level 2 for applied analytical problem solving without advanced algorithmic abstraction.",
    "W3: Scored at Level 2 or 3 due to compulsory embedded workplace or clinical practicum."
  ],
  "notScoreable": []
};

export default record;
