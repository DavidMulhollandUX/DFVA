// GENERATED FILE — DO NOT EDIT.
// Source: dfva/source/rubricV4.ts + dfva/source/evidence/*.json (panelCv4 blocks)
// Regenerate: npm --prefix scripts run dfva:gen-v4
import type { V4PanelC } from "../v4Meta";

const record: V4PanelC = {
  "instrument": "4.2-draft",
  "adaptiveness": 13,
  "workplace": 5,
  "C1": {
    "score": 3,
    "rationale": "Level 3: Structured communication of complex disciplinary concepts and technical arguments to diverse stakeholders in law.",
    "evidenceLines": [
      "**Institution**: The University of Queensland (UQ)",
      "**Program Code**: 2142",
      "- **Institution**: The University of Queensland (UQ)"
    ]
  },
  "C2": {
    "score": 3,
    "rationale": "Level 3: Critical evaluation of methodological assumptions, disciplinary arguments, and professional evidence in law.",
    "evidenceLines": [
      "**Academic Year**: 2026",
      "**Program Level**: Undergraduate",
      "- **Program Level**: Undergraduate"
    ]
  },
  "C3": {
    "score": 2,
    "rationale": "Level 2: Disciplinary problem solving applying quantitative, computational, or analytical frameworks within law.",
    "evidenceLines": [
      "Subject: Foundations of Law (LAWS1700)",
      "Subject: Law of Contract I: Principles of Contractual Agreement (LAWS1701)",
      "**AQF Level**: Refer to your component degrees for AQF level."
    ]
  },
  "C4": {
    "score": 3,
    "rationale": "Level 3: Interdisciplinary integration connecting core principles, professional practices, and evolving industry standards in law.",
    "evidenceLines": [
      "**Duration**: Not specified",
      "**Faculty**: of Humanities and Social Sciences",
      "- **AQF Level**: Refer to your component degrees for AQF level."
    ]
  },
  "C5": {
    "score": 2,
    "rationale": "Level 2: Applied investigation and empirical project work addressing real-world challenges in law.",
    "evidenceLines": [
      "Subject: Law of Contract II: Principles of Contractual Liability (LAWS1702)",
      "Subject: Principles of Public Law (LAWS1703)",
      "**Teaching Location**: St Lucia"
    ]
  },
  "W1": {
    "score": 2,
    "rationale": "Level 2: Professional conduct, ethical practice, and regulatory compliance aligned with standards in law.",
    "evidenceLines": [
      "**Attendance Mode**: Internal",
      "- **Institution**: The University of Queensland (UQ)",
      "- **Program Level**: Undergraduate"
    ]
  },
  "W2": {
    "score": 2,
    "rationale": "Level 2: Authentic tasks, applied case studies, and industry-aligned scenarios reflecting workplace practice in law.",
    "evidenceLines": [
      "**Handbook URL**: https://my.uq.edu.au/programs-courses/program.html?acad_prog=2142",
      "Subject: Law of Torts I (LAWS2702)",
      "- **AQF Level**: Refer to your component degrees for AQF level."
    ]
  },
  "W3": {
    "score": 1,
    "rationale": "Level 1: Academic coursework curriculum with elective industry engagements and experiential learning.",
    "evidenceLines": [
      "- **Duration**: Not specified",
      "- **Institution**: The University of Queensland (UQ)"
    ]
  },
  "gates": {
    "G1": {
      "result": "PASS",
      "rationale": "Postgraduate or undergraduate degree delivering comprehensive education in Bachelors of Arts / Laws.",
      "evidenceLines": [
        "- **Duration**: Not specified",
        "The LLB (Hons) fulfils the academic requirements for admission as a legal practitioner throughout Australia. After fulfilling the Practical Legal Training requirement, graduates may apply for admission through the Legal Practitioners Admission Board, the body responsible for making recommendations to the Supreme Court in respect of applications for admission to the legal profession in Queensland."
      ]
    },
    "G2": {
      "result": "PASS",
      "rationale": "Specialist academic program structured through sequential study in law.",
      "evidenceLines": [
        "Subject: Foundations of Law (LAWS1700)",
        "**Institution**: The University of Queensland (UQ)"
      ]
    }
  },
  "ambiguities": [
    "C3: Scored at Level 2 for applied analytical problem solving without advanced algorithmic abstraction.",
    "W3: Scored at Level 1 as external placement is an elective rather than a universal requirement."
  ],
  "notScoreable": []
};

export default record;
