...
function mockContext(user: { id: string } | null) {
  const jobStore = [
    { id: 'job-1', userId: 'user-a', handbookUrl: 'https://example.com/a', status: 'complete' },
    { id: 'job-2', userId: 'user-b', handbookUrl: 'https://example.com/b', status: 'processing' },
  ];
...
describe('getAssessmentJobs — auth guards', () => {
  it('returns 401 when user is not authenticated', async () => {
    ...
    await expect(getAssessmentJobs(undefined as any, ctx as any)).rejects
...
describe('getAssessmentJob — auth guards', () => {
  it('returns 401 when user is not authenticated', async () => {
    const ctx = mockContext(null);
    await expect(getAssessmentJob({ id: 'job-1' }, ctx as any)).rejects
      .toMatchObject({ statusCode: 401 });
  });
  ...
  it('returns 404 when job belongs to another user (hides existence)', async () => {
    const ctx = mockContext({ id: 'user-a' });
    await expect(getAssessmentJob({ id: 'job-2' }, ctx as any)).rejects
