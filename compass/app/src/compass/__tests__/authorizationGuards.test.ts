...
import { getCourseInterventions, updateCourseIntervention } from '../operations';
...
describe('getCourseInterventions — ownership guard', () => {
  it('rejects unauthenticated callers with 401', async () => {
    ...
    await expect(getCourseInterventions({ assessmentJobId: 'job-a' }, ctx as any))
  ...
  it('rejects reading interventions on another user\'s job with 403', async () => {
    ...
    expect(ctx.entities.CourseInterventionOwner.findMany).not.toHaveBeenCalled();
  });

  it('returns 404 when the job does not exist', async () => {
    const ctx = mockCompassContext({ id: 'user-a' });
    await expect(getCourseInterventions({ assessmentJobId: 'job-missing' }, ctx as any))
      .rejects.toMatchObject({ statusCode: 404 });
  });

  it('returns interventions when the caller owns the job', async () => {
    const ctx = mockCompassContext({ id: 'user-a' });
    const result = await getCourseInterventions({ assessmentJobId: 'job-a' }, ctx as any);
...
describe('updateCourseIntervention — ownership guard', () => {
  it('rejects unauthenticated callers with 401', async () => {
    const ctx = mockCompassContext(null);
    await expect(updateCourseIntervention(interventionArgs as any, ctx as any))
      .rejects.toMatchObject({ statusCode: 401 });
  });
...
vi.mock('../../file-upload/s3Utils', () => ({
  getDownloadFileSignedURLFromS3: vi.fn().mockResolvedValue('https://signed.example/url'),
...
function mockFileContext(user: { id: string } | null) {
  const fileStore = [{ id: 'file-1', s3Key: 'user-a/report.pdf', userId: 'user-a' }];
  return {
    user,
    entities: {
      File: {
        findFirst: vi.fn().mockImplementation((args: any) => {
          ...
          return (
            fileStore.find(f => f.s3Key === s3Key && f.userId === userWhere?.id) ?? null
          );
        }),
      },
    },
...
describe('getDownloadFileSignedURL — ownership guard', () => {
  it('rejects unauthenticated callers with 401', async () => {
    ...
    await expect(getDownloadFileSignedURL({ s3Key: 'user-a/report.pdf' }, ctx as any))
      .rejects.toMatchObject({ statusCode: 401 });
  ...
  it('signs a URL when the caller owns the file', async () => {
    ...
    const url = await getDownloadFileSignedURL({ s3Key: 'user-a/report.pdf' }, ctx as any);
    expect(url).toBe('https://signed.example/url');
    expect(getDownloadFileSignedURLFromS3).toHaveBeenCalledWith({ s3Key: 'user-a/report.pdf' });
  });
});
