...
describe("CreditStructureSchema", () => {
  it("accepts valid credit structure", () => {
    const result = CreditStructureSchema.safeParse({
      total: 200,
      core: 100,
      elective: 50,
    });
...
describe("SyllabusSnapshotSchema", () => {
  ...
  it("rejects empty subjects array", () => {
    const result = SyllabusSnapshotSchema.safeParse(
      makeValidSyllabus({ subjects: [] }),
    );
    expect(result.success).toBe(false);
  });
  ...
  it("rejects negative credit points", () => {
    const result = SyllabusSnapshotSchema.safeParse(
      makeValidSyllabus({ totalCreditPoints: -1 }),
    );
    expect(result.success).toBe(false);
  });
...
describe("validateSyllabus", () => {
  ...
  it("returns errors with path information", () => {
    const result = validateSyllabus(
      makeValidSyllabus({ totalCreditPoints: -5 }),
    );
    expect(result.success).toBe(false);
    if (!result.success)
      expect(
        result.errors.some((e: string) => e.includes("totalCreditPoints")),
      ).toBe(true);
  });
