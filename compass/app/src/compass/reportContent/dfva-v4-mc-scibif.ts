...
const content = {
  title: "Master of Science (Bioinformatics) (MC-SCIBIF) — Durability Report (v4 draft)",
  institution: "University of Melbourne",
  markdown: `# DFVA v4 DURABILITY REPORT: Master of Science (Bioinformatics) (MC-SCIBIF)
...
**Handbook vintage:** 2026 · **Source URL(s):** https://handbook.unimelb.edu.au/2026/courses/mc-scibif (course, attributes, structure) plus the overview and assessment pages of the compulsory subjects — 35 pages captured
**Course Director:** Vicky Perreau (Principal Coordinator)
...
> **This program is outside the v3.1 registry, but not outside Panel A.** Master of Science (Bioinformatics)
...
> a field grain basis (JSA HEO · 010999 Biological Sciences, n.e.c.). What such a
...
**Destination AI Exposure: 78.62.** Destination titles are mapped to the published Felten AI Occupational Exposure (AIOE) index and rescaled 0–100 by the Panel A procedure used for every other program; 0 of 13 titles carry medium mapping confidence. Basis: field-of-education occupation list, Jobs and Skills Australia Higher Education Outcomes — JSA HEO · 010999 Biological Sciences, n.e.c.; graduates of the whole field, not this program. The field-basis median is 83.21, so this program sits **at or below** it.
...
| C4 Life-long learning & transfer [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[[9]](https://www.nationalacademies.org/read/13398/chapter/2)[[10]](https://doi.org/10.1007/s10459-025-10413-y)[[2]](https://www.acses.edu.au/publication/australian-framework-for-artificial-intelligence-in-higher-education/) | 3 | Level 3's anchor is that 'The program documents structured progression toward independent learning: a self-scoped capstone or research project'. The course structure documents exactly that as a required curriculum component, not as an outcome claim: 50 credit points of Research Project subjects taken consecutively by all students, scoped with the project coordinator, gated behind a progression rule requiring 37.5 points of core first, with a required Professional Skills subject that cannot start until the project has begun. That is documented required structure rather than a graduate-attribute statement, which is what R2 forbids relying on. Level 2 is separately met (BINF90017 and COMP90038 assign work on data and problems beyond worked examples), so the item resolves at 3. |
...
- **G1 Disciplinary foundation:** PASS — The program documents a coherent specialist core with a staged prerequisite chain, which is the G1 PASS condition. 125 credit points of discipline core plus stream-specific compulsory subjects, a subject every stream must complete (BINF90002), explicit prerequisite chains (COMP90059 to COMP90038; MAST90044 to BINF90017; MAST20006 to MAST20005; MAST90044 or MAST20005 to BINF90001), a common second-year bioinformatics core, and a progression rule gating the research project behind 37.5 points of core. This is disciplinary identity, not interchangeable generic content.
- **G2 Decision-making under uncertainty:** PASS — G2 passes on the 'simulations/capstones/live projects with real uncertainty and accountability' route: a 50-point research project is required of every student, scoped with a project coordinator and staged across consecutive semesters — inquiry whose result is not known in advance. Defended trade-off decisions are additionally documented in assessed core work: BINF90002's outcome requires selecting an appropriate technology platform and analysis pipeline for different applications (assessed by essay, critical review and hurdled examination), and COMP90038's hurdled examination and semester project work address all ILOs over content that explicitly includes space and time trade-offs. This is beyond recall or scripted response.

## 4. MARKET EVIDENCE — Basis: reported

Condensed from [the market intelligence report](dfva-market-mc-scibif.md), whose §3 was
re-sourced in August 2026 against Australian Medicare claims analysis and the
structural-biology methods literature. Its headline confidence is **MEDIUM**.

**Destination job families.**

| Job family | Entry titles | AI substitution pressure | Skills rising in that family |
| --- | --- | --- | --- |
| Clinical bioinformatics | Clinical Bioinformatician, Variant Analyst, Genomic Data Scientist | Low-medium — automated interpretation is rising fast, but clinical validation and sign-off remain accountable human judgement | Automated variant interpretation with clinical validation, ACMG classification, clinical reporting pipelines and LIMS integration |
| Research bioinformatics | Bioinformatician, Computational Biologist, Research Assistant (Genomics) | Medium — pipeline operation compresses; question formulation and biological interpretation do not | Workflow orchestration (Nextflow, Snakemake, CWL), single-cell and spatial transcriptomics, cloud-scale genomics |
| Computational drug discovery | Computational Biologist, ML Scientist (Life Sciences) | Low — the scarce profile is biology *and* machine learning together, which is the part that is hard to staff | Protein language models, AlphaFold application, domain-specific ML in life sciences |

The §1 exposure value (78.62) rests on a **field-of-education** basis — the JSA HEO occupation
list for 010999 Biological Sciences, n.e.c. — so it describes graduates of the whole field,
not this program. Read it as placing these destinations below the field-basis median of
83.21, not as a measurement of this cohort. No source consulted reports destination volumes
specific to MC-SCIBIF.

**Signals and skill shifts.**

| Signal or shift | Direction | Bearing on the scored items |
| --- | --- | --- |
| Funded clinical genomics expanding — Medicare cancer-biomarker claims up ~5× 2010–2024 (5,130 → 25,374), with bioinformatics capability named as a system constraint | Rising | C4, C5, W1 |
| Under 40% of eligible patients receive exome/genome sequencing within two years of item creation — capacity-limited, not demand-satisfied | Rising, gated | W3, C4 |
| Workflow orchestration (Nextflow, Snakemake, CWL) as a baseline production expectation | Rising fast | W2, C1 |
| Tool operation without pipeline development skills | Declining, medium | W2, C2 |
| Manual variant annotation workflows against automated interpretation *with clinical validation* | Declining fast / rising fast | C2 |
| Single-cell and spatial transcriptomics (scRNA-seq, Visium) displacing bulk RNA-seq as the primary modality | Rising very fast | C5, C3 |
| Cloud-scale genomics (AWS Batch, Terra) over on-premise HPC administration | Rising fast | C3 |
| Protein language models and AlphaFold application — AlphaFold 3 (May 2024) extended prediction to complexes; ESM-2/ESM-3 remove the alignment step | Rising fast | C3, C5 |
| Generic data science without biological domain depth | Declining fast | C1, C5 |
| Clinical reporting pipelines and LIMS integration replacing Excel/manual genomics reporting | Rising, medium | W1, W2 |

**Confidence, restated from the market report.** Medium. The clinical-genomics demand signal
is the strongest element: it rests on national Medicare claims data rather than on inference,
and the same literature names bioinformatics capability as a distinct constraint. Two
weaknesses are declared in the source. The convergence claim — that bioinformatics is
becoming computational biology — carries no survey behind it and is stated there as
interpretation. And claims about ANZ employer hiring volumes were removed from §3 in the
re-sourcing rather than restated, so nothing here should be read as a count of advertised
roles.
...
and the score deltas are in the [improvement plan](dfva-v4-recommend-mc-scibif.md); rows
below are ordered by headroom, which is not the plan's P-lever order.

| Item | Score | Implication | Cost | Sources |
| --- | --- | --- | --- | --- |
| C2 Hybrid metacognition & evaluative judgement | 1/3 | Automated variant interpretation is rising fast while manual annotation declines; the value sits in judging machine output, and the program does not yet assess that judgement. | Adding appraisal-of-output tasks means re-weighting existing assessment rather than new content — the cheapest of the level-1 items to move. | [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[[4]](https://doi.org/10.1007/s10734-017-0220-3)[[3]](https://www.teqsa.gov.au/sites/default/files/2023-09/assessment-reform-age-artificial-intelligence-discussion-paper.pdf) |
| C3 Digital & AI literacy, including governance | 1/3 | Every machine-learning subject sits in the General Elective list while AI/ML in genomics and drug discovery is the named growth edge; protein-ML fluency appears in senior ads. | Moving ML from elective to core displaces an existing core subject, or lengthens the program — a structural decision, not a content tweak. | [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[13][[11]](https://doi.org/10.1145/3313831.3376727)[[12]](https://unesdoc.unesco.org/ark:/48223/pf0000391105)[[2]](https://www.acses.edu.au/publication/australian-framework-for-artificial-intelligence-in-higher-education/) |
| W1 Professional communication & conduct | 1/3 | Clinical reporting standards and ACMG variant classification are named hiring requirements; the absence of explicit clinical governance content limits hospital-network readiness. | Requires clinical-governance teaching the current staff profile may not cover; likely needs an industry-affiliated contributor. | [[19]](https://www.legislation.gov.au/F2021L00488)[[20]](https://www.qilt.edu.au/surveys/employer-satisfaction-survey-%28ess%29)[[21]](https://www.dewr.gov.au/skills-information-training-providers/resources/core-skills-work-developmental-framework)[[31]](https://www.naceweb.org/career-readiness/competencies/career-readiness-defined)[[5]](https://doi.org/10.1093/qje/qjx022)[[32]](https://doi.org/10.1086/504455) |
| W2 Authentic task design | 1/3 | Workflow orchestration is named in most production role descriptions and is a baseline expectation, against declining demand for tool operation without pipeline skills. | Orchestration tooling can be taught inside existing project subjects, but assessment must produce a running pipeline, which raises marking load. | [[22]](https://doi.org/10.1007/BF02504676)[[23]](https://doi.org/10.1080/02602938.2017.1412396)[[25]](https://doi.org/10.1016/j.stueduc.2021.101030)[26][[24]](https://doi.org/10.1080/02602938.2024.2404634) |
| W3 Work-situated learning | 1/3 | Clinical bioinformatician demand sits at named public-health networks; the improvement plan proposes employer partnerships on skill-development grounds. | Placement capacity is the binding constraint and depends on partners, not on curriculum design — the slowest lever here, and no employment-rate claim is made for it. | [[19]](https://www.legislation.gov.au/F2021L00488)[27][[29]](https://doi.org/10.1080/07294360.2016.1139551)[[28]](https://doi.org/10.1007/s10734-017-0216-z)[[35]](https://doi.org/10.1080/03075070903216635)[[30]](https://www.education.gov.au/australian-universities-accord/resources/final-report) |
| C1 Distributed cognition & relational capability | 2/3 | Role scope is described as expanding rather than specialising, with data-engineering tasks folded into bioinformatician descriptions; cross-disciplinary training is the durable profile. | Already at 2/3 — reaching 3 needs assessed team work across biology and computation, which is a timetabling and moderation cost more than a content one. | [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[[5]](https://doi.org/10.1093/qje/qjx022)[[7]](https://doi.org/10.1016/j.techfore.2016.08.019) |
| C5 Inquiry & evidence generation | 2/3 | Research question formulation and biological interpretation are the parts rated irreducible, and the field is described as shifting from tool operation toward research design. | Already at 2/3 and supported by the research project; the remaining gap is making design judgement explicit in assessment criteria. | [14][15][[7]](https://doi.org/10.1016/j.techfore.2016.08.019) |
| C4 Life-long learning & transfer | 3/3 | At the instrument maximum. Nothing in the market evidence suggests this is a weakness for this program. | None — no intervention proposed and none needed. | [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[[9]](https://www.nationalacademies.org/read/13398/chapter/2)[[10]](https://doi.org/10.1007/s10459-025-10413-y)[[2]](https://www.acses.edu.au/publication/australian-framework-for-artificial-intelligence-in-higher-education/) |

## 6. EVIDENCE CONFIDENCE & LIMITATIONS — Basis: reported

**Panel A metadata.** Basis tier **field** (field grain), n = 0, 13 destination titles, 0 at medium mapping confidence. Index variant AIOE-2021; destination titles mapped to the published Felten index. Basis note: field-of-education occupation list (ATO-linked), not this program's own graduates
