// Auto-generated from reports/*.md by scripts/dfva-content-generate.ts — do not edit by hand.
// Run: npm --prefix scripts run dfva:gen-content
const content = {
  title: "Master of Advanced Nursing Practice (MC-ANP) — Durability Report (v4 draft)",
  institution: "University of Melbourne",
  markdown: `# DFVA v4 DURABILITY REPORT: Master of Advanced Nursing Practice (MC-ANP)

**Instrument:** DFVA 4.2-draft — Panel C v4.2 on the TEQSA adaptive capabilities [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities) and the HESF generic and employment-related learning outcomes [[19]](https://www.legislation.gov.au/F2021L00488)
**Assessment date:** 2026-08-23
**Handbook vintage:** 2026 · **Source URL(s):** https://handbook.unimelb.edu.au/2026/courses/mc-anp (course, attributes, structure) plus the overview and assessment pages of the compulsory subjects — 35 pages captured
**Course Director:** Sophie Jones

> **Draft-instrument notice.** Panel C v4.2 is a working-draft instrument
> ([v4 recommendation §7](../docs/dfva-panelc-v4-recommendation.md), 2026-08-13;
> [v4.1 recommendation](../docs/dfva-panelc-v41-recommendation.md), 2026-08-14;
> [v4.2 amendment](../docs/dfva-rubric-evolution.md), 2026-08-16).
> Neither sub-score is comparable with any v3.1 value.

> **This program is outside the v3.1 registry, but not outside Panel A.** Master of Advanced Nursing Practice
> has never been assessed on v3.1, so it has no adaptiveness score on the published
> instrument. Exposure does not depend on the scoring instrument, so §1 reports a value
> computed by the identical Panel A procedure, on the basis stated there.

## 1. POSITION — Basis: measured × scored

**Destination AI Exposure: 76.38.** Destination titles are crosswalked to the published Felten AIOE index and rescaled 0–100 by the Panel A procedure used for every other program; 5 of 15 titles carry medium mapping confidence. Basis: measured on the program's own alumni destination record (n = 69, 15 titles). The portfolio median is 90.9, so this program sits **at or below** it.

**Curriculum Adaptiveness: 9 / 15.** Scored on Panel C v4.2 sub-scale A [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities) from 2026 handbook evidence: anchors are declarative statements about documented curriculum evidence, level 3 requires assessment evidence, and ambiguous evidence resolves to the lower level.

**Workplace Practice: 5 / 9.** Scored on sub-scale W [[19]](https://www.legislation.gov.au/F2021L00488) in the same pass, from the same extract. The position axis remains adaptiveness alone; the workplace sub-score is reported beside it, not folded into the axis, and the two are never added together.

**Position: Low exposure · high adaptiveness.** Assigned against the v4 medians (exposure 90.9, adaptiveness 9), computed from all 34 reference-cohort programs re-scored on this instrument. This is the deterministic median rule the site applies, not a probabilistic stability class.

## 2. PANEL C v4.2 SCORECARD — Basis: scored

The two sub-scales are kept separate and carry their own subtotals. Panel C has no
combined total: the sub-scores are reported side by side and never added.

| Item | Score | Rationale (anchor-referenced) |
| --- | --- | --- |
| C1 Distributed cognition & relational capability [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[[5]](https://doi.org/10.1093/qje/qjx022)[[7]](https://doi.org/10.1016/j.techfore.2016.08.019) | 2 | Level 2's anchor ('at least one core unit assesses collaborative practice — group projects with individual accountability, client or stakeholder work, interprofessional activity') is satisfied in compulsory core subject NURS90069 Clinical Leadership in Context, which assesses 'Group Project Part A: Interactive Oral Presentation' (35%) and 'Group Project Part B: Peer Feedback (Invigilated & Synchronous)' (15%) alongside individual accountability through 'Individual Clinical Leadership Project' Part A (Plan, 15%) and Part B (Report, 35%). Level 3 is not reached: while collaborative practice occurs in core coursework and workshops, the extract documents no assessment requiring students to coordinate work across people AND tools or AI systems, nor any assessed division of labour between people and automated systems. |
| C2 Hybrid metacognition & evaluative judgement [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[[4]](https://doi.org/10.1007/s10734-017-0220-3)[[3]](https://www.teqsa.gov.au/sites/default/files/2023-09/assessment-reform-age-artificial-intelligence-discussion-paper.pdf) | 2 | Level 2's anchor ('core assessment includes criterion-referenced appraisal of quality: peer review, structured critique, portfolio with standards, marking against exemplars') is met in compulsory core subject NURS90069 Clinical Leadership in Context by the assessed 'Group Project Part B: Peer Feedback (Invigilated & Synchronous)' (1000 words equivalent, 15%), an invigilated peer review appraisal of peers' presentations. In addition, compulsory capstone NURS90120 Evidence in Practice formally assesses 'critically appraising research using relevant appraisal frameworks'. Level 3 is not reached: no assessment requires students to document and justify reliance decisions (when they relied on or overrode a tool, source or collaborator), defend judgements of AI-output quality, or evidence strategy adjustment over time. |
| C3 Digital & AI literacy, including governance [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[13][[11]](https://doi.org/10.1145/3313831.3376727)[[12]](https://unesdoc.unesco.org/ark:/48223/pf0000391105)[[2]](https://www.acses.edu.au/publication/australian-framework-for-artificial-intelligence-in-higher-education/) | 1 | Level 1's anchor ('AI or digital tools appear only as electives or as tool operation/training — the operational level TEQSA identifies as non-durable') describes the documented evidence. Digital tools appear in course generic skills ('web based media', 'information and communication technologies') and as tool operation in compulsory core subject NURS90120 Evidence in Practice ('systematic search of electronic databases', 'application of information technology skills, including software systems'), with digital health electives available ('ISYS90069 Digital Transformation of Health', 'POPH90308 Indigenous Data Governance in Health'). Level 0 is negated by these program-level generic skills and core subject outcomes. Level 2 is not reached: no core unit addresses AI capabilities AND limitations/ethics with use limits discussed and assessed. |
| C4 Life-long learning & transfer [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[[9]](https://www.nationalacademies.org/read/13398/chapter/2)[[10]](https://doi.org/10.1007/s10459-025-10413-y)[[2]](https://www.acses.edu.au/publication/australian-framework-for-artificial-intelligence-in-higher-education/) | 3 | Level 3's anchor ('the program documents structured progression toward independent learning: a self-scoped capstone or research project, or assessed identification of one's own knowledge gaps together with the plan to close them') is met across the compulsory core sequence. The program mandates a 25-credit-point capstone sequence where NURS90120 Evidence in Practice serves as the prerequisite to NURS90070 Implementing Evidence for Practice, in which students self-scope a clinical practice change project ('Synthesise relevant literature to support a proposed clinical practice change within a healthcare setting', assessed by a 3000-word written assignment and interactive oral presentation). In addition, compulsory core NURS90069 Clinical Leadership in Context explicitly requires students to 'identify their own learning and development needs with respect to clinical leadership and, where necessary, negotiate access to a range of resources to enable them to meet their learning objectives', assessed via the Individual Clinical Leadership Project Plan (Part A) and Report (Part B). Level 2's transfer requirements (application to novel practice problems) are also independently satisfied. |
| C5 Inquiry & evidence generation [14][15][[7]](https://doi.org/10.1016/j.techfore.2016.08.019) | 1 | Level 1's anchor ('Introductory methods unit; literature-review assessment') matches the common core required of all students. Compulsory unit NURS90120 Evidence in Practice provides introductory critical appraisal and database search methods ('identifying and critically appraising research as evidence to inform their clinical practice', assessed by 1000-word and 2500-word written assessments), followed by secondary evidence synthesis in compulsory capstone NURS90070 ('Synthesise relevant literature to support a proposed clinical practice change'). Level 2 is not met in the common core because students do not conduct primary data collection. Level 3 is strictly disqualified because a primary research project (NURS90164/165 Independent Study Project Parts 1 & 2) exists only as an optional Research stream rather than being REQUIRED of all students (Coursework option requires 50 cp of core selective and electives without primary data collection). |
| **Adaptive capabilities** | **9 / 15** | |
| W1 Professional communication & conduct [[19]](https://www.legislation.gov.au/F2021L00488)[[20]](https://www.qilt.edu.au/surveys/employer-satisfaction-survey-%28ess%29)[[21]](https://www.dewr.gov.au/skills-information-training-providers/resources/core-skills-work-developmental-framework)[[31]](https://www.naceweb.org/career-readiness/competencies/career-readiness-defined)[[5]](https://doi.org/10.1093/qje/qjx022)[[32]](https://doi.org/10.1086/504455) | 2 | Level 2's anchor ('At least one core assessment requires a recognised professional genre or an audience beyond the teaching team — client brief, consultancy or policy advice, clinical handover, pitch, public-facing artefact — judged against criteria drawn from professional practice') is met in compulsory core units: NURS90069 assesses a Clinical Leadership Project Plan (Part A) and Report (Part B) alongside an Interactive Oral Presentation; NURS90086 Quality and Safety in Healthcare assesses a 3000-word clinical incident investigation and QI project with an oral defence (500 words equivalent); and NURS90070 assesses an evidence-based clinical practice change proposal and interactive oral presentation. Level 3 is not met: compulsory core assessments are delivered to and judged by the academic teaching team and peers; external practitioner-assessed clinical assessments (such as NURS90068 MiniCEX) sit in elective specialty subjects rather than the core required of all students. |
| W2 Authentic task design [[22]](https://doi.org/10.1007/BF02504676)[[23]](https://doi.org/10.1080/02602938.2017.1412396)[[25]](https://doi.org/10.1016/j.stueduc.2021.101030)[26][[24]](https://doi.org/10.1080/02602938.2024.2404634) | 2 | Level 2's anchor ('At least one core assessment reproduces a professional task end to end: a real or realistic problem, producing the artefact a practitioner would produce, judged against criteria drawn from practice') is satisfied across the core curriculum: NURS90069 assesses an individual clinical leadership project plan and practice reform report; NURS90086 assesses an authentic clinical incident review, root cause analysis, and quality improvement plan with oral defence; and NURS90070 assesses an evidence implementation plan analysing real organisational enablers and barriers. Level 3 is not taken: while these tasks form the core assessment spine, they are internally/classroom framed without externally supplied ambiguous client briefs or consequential external stakeholders. In accordance with the non-negotiable rule resolving ambiguity downward, the score is 2. |
| W3 Work-situated learning [[19]](https://www.legislation.gov.au/F2021L00488)[27][[29]](https://doi.org/10.1080/07294360.2016.1139551)[[28]](https://doi.org/10.1007/s10734-017-0216-z)[[35]](https://doi.org/10.1080/03075070903216635)[[30]](https://www.education.gov.au/australian-universities-accord/resources/final-report) | 1 | Level 1's anchor ('Work-situated learning exists only as an elective, an optional internship, or an unassessed extracurricular or careers activity') is matched by the course structure. The 50 credit points of compulsory core subjects (NURS90069, NURS90086, NURS90120, NURS90070) are on-campus units without a mandatory placement or assessed own-practice requirement. Work-situated clinical practice units with workplace supervisor assessment (such as NURS90067 and NURS90068 MiniCEX patient health assessments assessed by clinical supervisors in the practice environment, NURS90101/119, and NURS90105) exist only as discipline electives. Level 0 is negated because work-situated learning appears in elective options; Level 2 is unmet because no work-situated learning unit is in the compulsory core required of all students. |
| **Workplace practice** | **5 / 9** | |

**C1 Distributed cognition & relational capability — 2/3**

> Individual Clinical Leadership Project (Part A): Clinical Leadership Project Plan
>
> Group Project Part A: Interactive Oral Presentation
>
> Group Project Part B: Peer Feedback (Invigilated & Synchronous)
>
> Individual Clinical Leadership Project (Part B): Clinical Leadership Project Report
>
> Explain how leaders can support effective team dynamics and manage conflict.
>
> Capacity to work collaboratively with people from diverse communities to achieve goals

**C2 Hybrid metacognition & evaluative judgement — 2/3**

> Group Project Part B: Peer Feedback (Invigilated & Synchronous)
>
> 1000 words (equivalent)
>
> Demonstrating knowledge and skills in critically appraising research using relevant appraisal frameworks
>
> Apply and critique quality improvement methods and tools to suit the context of the improvement needed at a local level.

**C3 Digital & AI literacy, including governance — 1/3**

> • the ability to communicate scientific knowledge through oral, written and web based media
>
> be adept at learning in a range of ways, including through information and communication technologies
>
> Demonstrating how to undertake a systematic search of electronic databases
>
> Demonstrate the application of information technology skills, including software systems
>
> Where appropriate, advanced working skills in the application of computer systems and software and a receptiveness to the opportunities offered by new technologies;
>
> ISYS90069	Digital Transformation of Health
>
> POPH90308	Indigenous Data Governance in Health

**C4 Life-long learning & transfer — 3/3**

> All students must complete:
>
> 25 credit points of compulsory subjects
>
> 25 credit points of capstone subjects
>
> This subject is designed to build on students’ knowledge and critical appraisal skills gained in the pre-requisite subject, Evidence in Practice (NURS90120), to implement the best available evidence into clinicians’ practice.
>
> Synthesise relevant literature to support a proposed clinical practice change within a healthcare setting
>
> Formulate appropriate strategies to implement research evidence into clinical practice within a healthcare setting
>
> The subject will also assist students to identify their own learning and development needs with respect to clinical leadership and, where necessary, negotiate access to a range of resources to enable them to meet their learning objectives.
>
> Individual Clinical Leadership Project (Part A): Clinical Leadership Project Plan
>
> Individual Clinical Leadership Project (Part B): Clinical Leadership Project Report

**C5 Inquiry & evidence generation — 1/3**

> This subject is designed to meet the needs of nurses and other clinically-based healthcare professionals seeking further education in identifying and critically appraising research as evidence to inform their clinical practice.
>
> Describing the role of evidence-based practice to inform health-care decisions
>
> Demonstrating how to undertake a systematic search of electronic databases
>
> Identify and justify appropriate sources of information to answer clinical questions
>
> Evaluating, synthesising and summarising research in health-related disciplines
>
> Synthesise relevant literature to support a proposed clinical practice change within a healthcare setting
>
> Written assignment
>
> Coursework option
>
> OR
>
> Research option
>
> Design and conduct a research project, or elements of a project, in the area of nursing practice
>
> Project Report including scholarly work suitable for publication

**W1 Professional communication & conduct — 2/3**

> Individual Clinical Leadership Project (Part A): Clinical Leadership Project Plan
>
> Group Project Part A: Interactive Oral Presentation
>
> Group Project Part B: Peer Feedback (Invigilated & Synchronous)
>
> Individual Clinical Leadership Project (Part B): Clinical Leadership Project Report
>
> Oral Presentation
>
> Written assignment (2500 words) & oral defence (500 words equivalent)
>
> Individual interactive oral presentation
>
> Written assignment

**W2 Authentic task design — 2/3**

> Individual Clinical Leadership Project (Part A): Clinical Leadership Project Plan
>
> Individual Clinical Leadership Project (Part B): Clinical Leadership Project Report
>
> Develop initiatives that support healthcare systems to address issues impacting patient care and outcomes.
>
> Critically analyse the causes of clinical incidents, near misses and environmental hazards.
>
> Apply and critique quality improvement methods and tools to suit the context of the improvement needed at a local level.
>
> Written assignment (2500 words) & oral defence (500 words equivalent)
>
> Synthesise relevant literature to support a proposed clinical practice change within a healthcare setting
>
> Formulate appropriate strategies to implement research evidence into clinical practice within a healthcare setting
>
> Analyse potential enablers and barriers in implementing research evidence into clinical practice within a healthcare setting

**W3 Work-situated learning — 1/3**

> The Master of Advanced Nursing Practice is a graduate coursework degree consisting of 100 points, normally taking twelve months of full-time study or part-time equivalent. Masters prepared nurses assume responsibility for establishing new professional roles, devising new models of care and leading dynamic changes to practice.
>
> 25 credit points of compulsory subjects
>
> 25 credit points of capstone subjects
>
> Students will also require clinical supervisors in their practice environment to support their learning, in particular the development of advanced specialty assessment skills and to assess their clinical performance.
>
> Six patient health assessments equivalent to 500 words each
>
> Students will use clinical supervisors in their practice environment to support their learning and assess their performance, in particular development of practical assessment skills, use of diagnostic tests and interpret findings in relation to undertaking a physical health assessment.
>
> Live Performance (MiniCEX) - 4 x MiniCEX patient health assessments equivalent to 500 words each
>
> NURS90101	Mental Health Nursing Practice 1
>
> NURS90119	Mental Health Nursing Practice 2
>
> NURS90105	Chronic Disease Management: In Practice

## 3. GATES — Basis: scored

- **G1 Disciplinary foundation:** PASS — PASS on the anchor's all-compulsory specialist core and staged prerequisite chain routes: the program mandates 50 credit points of core advanced nursing coursework (leadership, quality/safety, evidence appraisal and translation), with NURS90120 explicitly staged as the prerequisite to capstone NURS90070, building progressive technical and methodological depth toward advanced nursing practice.
- **G2 Decision-making under uncertainty:** PASS — PASS on the anchor's defended trade-off decisions and uncertainty route: assessments require defended clinical risk and governance decisions under genuine healthcare complexity and uncertainty, including a 3000-word incident analysis and QI proposal with a live oral defence in NURS90086, an interactive oral defence of an evidence implementation plan addressing organizational barriers in NURS90070, and individual leadership project planning and reporting in NURS90069.

## 4. MARKET EVIDENCE — Basis: reported

| Job family | Entry titles | AI substitution pressure | Skills rising in that family |
|---|---|---|---|
| Advanced Practice Nursing & Specialist Practice | Clinical Nurse Specialist, Clinical Nurse Consultant, Nurse Specialist, Psychiatric Registered Nurse | LOW — physical assessment, bedside clinical reasoning, and therapeutic relational capability are irreducible | Complex diagnostic reasoning, chronic disease management, digital health literacy, trauma-informed care |
| Nurse Practitioner / Autonomous Practice | Nurse Practitioner, Candidate Nurse Practitioner, Specialty NP | LOW — autonomous statutory and clinical accountability cannot be delegated to automated systems | Prescribing pharmacology, advanced physical assessment, clinical diagnostic reasoning, interprofessional negotiation |
| Clinical Nursing Leadership & Health Services Management | Nurse Unit Manager, Nurse Manager, National Manager Palliative Care | LOW — conflict resolution, operational risk governance, and team leadership are human-dependent | Clinical risk governance, healthcare data analytics, change management, workforce planning |
| Clinical Education & Practice Development | Clinical Nurse Educator, Clinical Nurse Facilitator, Practice Development Nurse | LOW–MEDIUM — didactic materials and scenario generation can be AI-assisted; clinical mentorship and live evaluation remain human | Simulation facilitation, digital learning technologies, workplace-based assessment design |
| Clinical Research Coordination & Evidence Translation | Nurse Research Coordinator, Clinical Trial Nurse, Senior Clinical Research Nurse | LOW–MEDIUM — data management and literature search augmented by AI; clinical safety monitoring and protocol fidelity require human oversight | Good Clinical Practice (GCP), clinical trial informatics (REDCap), research translation, ethical governance |

The **Destination AI Exposure of 76.38** sits at or below the portfolio median of 90.9. It is **measured** on this program's own alumni destination record (n = 69, 15 titles), where 5 of 15 titles carry medium mapping confidence. Titles such as Nurse Unit Manager and National Manager Palliative Care map to Medical and Health Services Managers (AIOE 93.63), while clinical and specialist nursing roles map to Registered Nurses (AIOE 69.07) and Nurse Practitioners (AIOE 68.13).

| Signal or shift | Direction | Bears on |
|---|---|---|
| EMR integration, digital workflow mastery, and critical evaluation of Clinical Decision Support Systems (CDSS) | Rising | C3, C2 |
| Non-delegable human practitioner clinical and legal accountability under Ahpra and NMBA regulatory guidance | Rising | C3, W1 |
| Interprofessional collaboration, clinical leadership, and systems thinking across healthcare teams | Rising | C1 |
| Clinical risk governance, root cause analysis, and defended oral quality improvement proposals under NSQHS standards | Rising | W1, W2 |
| Practice-based clinical inquiry, empirical audit, and structured research translation in clinical workflows | Rising | C5, W3 |
| Routine unintegrated paper documentation, uncritical reliance on standard protocols, and siloed unit task execution | Declining | C1, C2, W2 |

**Evidence confidence, restated from the market report.** Confidence is **MEDIUM–HIGH** overall. Regulatory accountability and AI governance frameworks are HIGH, established directly from published NMBA standards, Ahpra & National Boards joint guidance, and ADHA frameworks. Public hospital workforce demand and clinical leadership recruitment are HIGH, supported by Victorian health service structures and enterprise agreements. Digital health capability standards are MEDIUM–HIGH. Specific clinical research nurse hiring volumes are MEDIUM, inferred from biomedical precinct activity. Graduate destination tracking rests on the program's own alumni record (n = 69, 15 titles, exact basis), where several senior management titles reflect career seniority.

## 5. CURRICULUM IMPLICATIONS — Basis: inferred

This section argues from the evidence above; it is interpretation, not observation.

Each row states an option and what it costs, not a directive. Anchor text, sequencing
and the score deltas are in the [improvement plan](dfva-v4-recommend-mc-anp.md); rows
below are ordered by headroom, which is not the plan's P-lever order.

| Item | Score | Implication | Cost | Sources |
| --- | --- | --- | --- | --- |
| C3 Digital & AI literacy, including governance | 1/3 | National digital health frameworks (ADHA 2026) and Ahpra AI principles require advanced nurses to critically evaluate Clinical Decision Support Systems (CDSS), algorithmic bias, and data governance in clinical workflows rather than treating technology as basic tool operation. | Embedding a dedicated clinical AI/CDSS governance module and assessment in compulsory core unit NURS90086 or NURS90070; requires faculty capability in digital health and clinical informatics. | [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[13][[11]](https://doi.org/10.1145/3313831.3376727)[[12]](https://unesdoc.unesco.org/ark:/48223/pf0000391105)[[2]](https://www.acses.edu.au/publication/australian-framework-for-artificial-intelligence-in-higher-education/) |
| C5 Inquiry & evidence generation | 1/3 | Expanding clinical research coordination and evidence translation roles require advanced practitioners to design and execute empirical inquiries rather than solely synthesising published literature. | Embedding a compulsory clinical audit or QI inquiry task with primary data collection into capstone NURS90070, or requiring the empirical research stream (NURS90164/165) for all students; increases supervisory and marking load. | [14][15][[7]](https://doi.org/10.1016/j.techfore.2016.08.019) |
| W3 Work-situated learning | 1/3 | Advanced clinical leadership and practice reform are situated within complex healthcare environments; for a cohort already practising as registered nurses, assessing own-practice clinical implementation bridges workplace learning without creating unfunded placement burdens. | Structuring an assessed own-practice clinical implementation component with workplace stakeholder feedback and documented outcome measures in NURS90070; requires formalising workplace supervisor accountability. | [[19]](https://www.legislation.gov.au/F2021L00488)[27][[29]](https://doi.org/10.1080/07294360.2016.1139551)[[28]](https://doi.org/10.1007/s10734-017-0216-z)[[35]](https://doi.org/10.1080/03075070903216635)[[30]](https://www.education.gov.au/australian-universities-accord/resources/final-report) |
| C1 Distributed cognition & relational capability | 2/3 | Advanced nursing leadership requires coordinating multidisciplinary healthcare teams and governing the division of clinical labour between human practitioners and automated CDSS or AI documentation systems. | Modifying the existing collaborative leadership project in NURS90069 to assess team coordination across people AND digital/AI systems; low curricular friction within existing subject learning activities. | [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[[5]](https://doi.org/10.1093/qje/qjx022)[[7]](https://doi.org/10.1016/j.techfore.2016.08.019) |
| C2 Hybrid metacognition & evaluative judgement | 2/3 | Ahpra's mandate for non-delegable clinical accountability requires practitioners to systematically document, critique, and justify when they rely on or override automated decision support tools and peer feedback. | Adding an assessed reliance justification requirement and evaluative audit to NURS90070 or NURS90086; negligible cost, requires rubric refinement and exemplar release. | [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[[4]](https://doi.org/10.1007/s10734-017-0220-3)[[3]](https://www.teqsa.gov.au/sites/default/files/2023-09/assessment-reform-age-artificial-intelligence-discussion-paper.pdf) |
| W1 Professional communication & conduct | 2/3 | Clinical governance and incident investigation demand defended oral communication and professional conduct evaluated against authentic healthcare industry standards. | Convening an external panel of healthcare service leaders (e.g. Directors of Nursing, Quality & Safety Leads) to co-judge the live oral defence in NURS90086; involves panel coordination and external scheduling. | [[19]](https://www.legislation.gov.au/F2021L00488)[[20]](https://www.qilt.edu.au/surveys/employer-satisfaction-survey-%28ess%29)[[21]](https://www.dewr.gov.au/skills-information-training-providers/resources/core-skills-work-developmental-framework)[[31]](https://www.naceweb.org/career-readiness/competencies/career-readiness-defined)[[5]](https://doi.org/10.1093/qje/qjx022)[[32]](https://doi.org/10.1086/504455) |
| W2 Authentic task design | 2/3 | Preparing graduates for high-stakes healthcare leadership requires assessment tasks governed by real, uncurated healthcare challenges carrying authentic operational and resource constraints. | Sourcing live, ambiguous clinical incident or QI briefs from partner Victorian health services for NURS90086; requires ongoing industry liaison and case curation. | [[22]](https://doi.org/10.1007/BF02504676)[[23]](https://doi.org/10.1080/02602938.2017.1412396)[[25]](https://doi.org/10.1016/j.stueduc.2021.101030)[26][[24]](https://doi.org/10.1080/02602938.2024.2404634) |
| C4 Life-long learning & transfer | 3/3 | The compulsory capstone sequence (NURS90120 prerequisite to self-scoped NURS90070 practice change) combined with metacognitive leadership development plans in NURS90069 satisfies the highest instrument standard. | None — no intervention proposed and none needed. | [[1]](https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities)[[9]](https://www.nationalacademies.org/read/13398/chapter/2)[[10]](https://doi.org/10.1007/s10459-025-10413-y)[[2]](https://www.acses.edu.au/publication/australian-framework-for-artificial-intelligence-in-higher-education/) |

## 6. EVIDENCE CONFIDENCE & LIMITATIONS — Basis: reported

**Panel A metadata.** Basis tier **exact** (measured), n = 69, 15 destination titles, 5 at medium mapping confidence. Index variant AIOE-2021; crosswalk \`data/aioe/v31_extension_crosswalk.csv\`.

**Panel C provenance.** Scored from the 2026 handbook capture (35 pages) and verified on 2026-08-23: adversarial refutation of every level-3 score and every gate PASS (run), verbatim evidence check (run), and the construct-boundary pass.

**Ambiguities and construct-boundary notes** (5):

- C1 vs W1 (one construct, one home): NURS90069's group presentation and invigilated peer feedback were decomposed by construct: coordination of group presentation and individual accountability scored in C1; peer appraisal of presentation quality scored in C2; professional spoken genre and communication scored in W1.
- C2, levels 1 vs 2: NURS90069 contains an explicit 15% assessed component 'Group Project Part B: Peer Feedback (Invigilated & Synchronous)' which satisfies level 2's peer review / structured critique anchor, supported by NURS90120's formal research appraisal frameworks. Level 3 is absent due to lack of AI reliance/override justification.
- C5, levels 1 vs 3: A primary empirical research project exists in the program (NURS90164/165 Independent Study Project Parts 1 & 2, 25 points with a 6000-word publication-standard report), which would satisfy level 2 or 3 if compulsory, but because it sits in an optional Research Option and is not required for coursework-stream students, Level 3's 'REQUIRED (not one route among several)' clause strictly forces the score to the common core level 1.
- W2, levels 2 vs 3: Core assessments (clinical leadership project, incident review with oral defence, evidence translation project) form an authentic professional assessment spine across the core, but the problems are internally framed without externally supplied client briefs or consequential external audiences. Ambiguity resolved down to 2 per non-negotiable rules.
- W3, levels 1 vs 2/3: Clinical supervisor-assessed workplace assessments (such as MiniCEX in NURS90068) exist within elective subjects for specialty streams, but no work-situated learning is mandatory in the 50 credit points of common core subjects required of all students. Scored at level 1 (electives only).

**Standing v4 limitations.**

- Scores describe **documented curriculum intent**, not demonstrated graduate
  capability; the extrapolation warrant is constructive alignment, stated as an
  assumption (Kane [17]).
- Indigenous data governance is not scored as a distinct construct; C3 level 3
  counts it where taught (CARE Principles; Lodge et al. 2025 Principle 4 [[2]](https://www.acses.edu.au/publication/australian-framework-for-artificial-intelligence-in-higher-education/)).
- Perception/manipulation (Frey & Osborne's third bottleneck [[7]](https://doi.org/10.1016/j.techfore.2016.08.019)) is unscored.
- A high W3 score describes curriculum design, not student welfare: required
  placements can disadvantage students who cannot afford unpaid work or
  relocation (Accord [[30]](https://www.education.gov.au/australian-universities-accord/resources/final-report); Fawns et al. [[24]](https://doi.org/10.1080/02602938.2024.2404634) on ableism and elitism).
- W2 is scored against each discipline's own criterion situation [[22]](https://doi.org/10.1007/BF02504676), so
  cross-disciplinary W2 comparison is weaker than within-discipline comparison.
- W3 excludes simulation by design (simulation scores in W2), so programs whose
  professional norm is high-fidelity simulation may score low on W3 for a
  defensible pedagogic choice. Read the two sub-scores together.
- The authentic-assessment employability evidence rests largely on student
  self-report [[25]](https://doi.org/10.1016/j.stueduc.2021.101030); W2's warrant is documented design fidelity plus the
  assessment-drives-learning mechanism [26], not demonstrated employment effects.
- Work-integrated learning is NOT claimed to raise employment rates — Jackson &
  Collings [[28]](https://doi.org/10.1007/s10734-017-0216-z) found it does not. W3's warrant is skill development, employment
  relevance and professional identity formation [[29]](https://doi.org/10.1080/07294360.2016.1139551).

## REFERENCES

1. Lodge JM, de Barba P, Ainscough L, et al. (2026). Assuring quality learning in a gen AI-integrated future: The role of adaptive capabilities. TEQSA. https://www.teqsa.gov.au/guides-resources/resources/corporate-publications/assuring-quality-learning-gen-ai-integrated-future-role-adaptive-capabilities
2. Lodge JM, et al. (2025). Australian Framework for Artificial Intelligence in Higher Education. ACSES. (Principles 4 & 6.) https://www.acses.edu.au/publication/australian-framework-for-artificial-intelligence-in-higher-education/
3. Lodge JM, Howard S, Bearman M, Dawson P & Associates (2023). Assessment reform for the age of artificial intelligence. TEQSA. https://www.teqsa.gov.au/sites/default/files/2023-09/assessment-reform-age-artificial-intelligence-discussion-paper.pdf
4. Tai J, Ajjawi R, Boud D, Dawson P, Panadero E (2018). Developing evaluative judgement: enabling students to make decisions about the quality of work. Higher Education 76:467–481. https://doi.org/10.1007/s10734-017-0220-3
5. Deming DJ (2017). The growing importance of social skills in the labor market. Quarterly Journal of Economics 132(4):1593–1640. https://doi.org/10.1093/qje/qjx022
6. Deming DJ, Noray K (2020). Earnings dynamics, changing job skills, and STEM careers. Quarterly Journal of Economics 135(4):1965–2005. https://doi.org/10.1093/qje/qjaa021
7. Frey CB, Osborne MA (2017). The future of employment: how susceptible are jobs to computerisation? Technological Forecasting & Social Change 114:254–280. https://doi.org/10.1016/j.techfore.2016.08.019
8. Brynjolfsson E, Mitchell T, Rock D (2018). What can machines learn, and what does it mean for occupations and the economy? AEA Papers & Proceedings 108:43–47. (The SML rubric — the declarative-anchor form R1 copies.) https://doi.org/10.1257/pandp.20181019
9. Pellegrino JW, Hilton ML (eds) (2012). Education for Life and Work: Developing Transferable Knowledge and Skills in the 21st Century. National Research Council. https://www.nationalacademies.org/read/13398/chapter/2
10. Measuring adaptive expertise and adaptive performance in (becoming) healthcare professionals: a scoping review of measurement instruments (2025). Advances in Health Sciences Education. https://doi.org/10.1007/s10459-025-10413-y
11. Long D, Magerko B (2020). What is AI literacy? Competencies and design considerations. CHI 2020. https://doi.org/10.1145/3313831.3376727
12. UNESCO (2024). AI competency framework for students. https://unesdoc.unesco.org/ark:/48223/pf0000391105
13. European Commission JRC — DigComp (Digital Competence Framework for Citizens); basis of the Australian Digital Capability Framework (DEWR, 2022).
14. Boyer EL (1990). Scholarship Reconsidered: Priorities of the Professoriate. Carnegie Foundation.
15. Brew A (2006). Research and Teaching: Beyond the Divide. Palgrave Macmillan.
16. Knight FH (1921). Risk, Uncertainty and Profit. Houghton Mifflin.
17. Kane MT (2013). Validating the interpretations and uses of test scores. Journal of Educational Measurement 50(1):1–73. (The validity-argument frame for the v4 evidence plan.)
18. Woods L, Lyons K, et al. (2026). Assessing the effectiveness of artificial intelligence education and training for healthcare workers: a systematic review. BMC Medical Education 26:549. (The construct-validity critique v4 answers.) https://doi.org/10.1186/s12909-026-08969-3
19. Higher Education Standards Framework (Threshold Standards) 2021, F2021L00488. Clause 1.4.2(b)–(d) (generic and employment-related learning outcomes, applied in disciplinary context), 1.4.3 (assessment must confirm the outcomes), 5.4.1 (work-integrated learning and placements are quality assured, including supervision). https://www.legislation.gov.au/F2021L00488
20. QILT Employer Satisfaction Survey (ESS) — national employer ratings across five graduate attribute domains: foundation, adaptive, collaborative, technical and employability skills. (2025 national results: technical 94.0%, foundation 93.3%, adaptive 90.7%, collaborative 88.6%, employability 86.2%.) https://www.qilt.edu.au/surveys/employer-satisfaction-survey-%28ess%29
21. Core Skills for Work Developmental Framework (Australian Government, 2013). Three clusters — navigate the world of work, interact with others, get the work done — over ten skill areas and five performance stages. Anchor-content source; a VET framework, not a higher-education construct authority. https://www.dewr.gov.au/skills-information-training-providers/resources/core-skills-work-developmental-framework
22. Gulikers JTM, Bastiaens TJ, Kirschner PA (2004). A five-dimensional framework for authentic assessment. Educational Technology Research and Development 52(3):67–86. (Task, physical context, social context, result/form, criteria; authenticity is a continuum defined against the criterion situation — design rule R4.) https://doi.org/10.1007/BF02504676
23. Villarroel V, Bloxham S, Bruna D, Bruna C, Herrera-Seda C (2018). Authentic assessment: creating a blueprint for course design. Assessment & Evaluation in Higher Education 43(5):840–854. (Realism, cognitive challenge, evaluative judgement — the third dimension is DFVA C2, not W2.) https://doi.org/10.1080/02602938.2017.1412396
24. Fawns T, Bearman M, Dawson P, Nieminen JH, Ashford-Rowe K, Willey K, Jensen LX, Damşa C, Press N (2024). Authentic assessment: from panacea to criticality. Assessment & Evaluation in Higher Education 50(3):396–408. (The label risks becoming a 'thought-terminating cliché' — the constraint behind R4.) https://doi.org/10.1080/02602938.2024.2404634
25. Sokhanvar Z, Salehi K, Sokhanvar F (2021). Advantages of authentic assessment for improving the learning experience and employability skills of higher education students: a systematic literature review. Studies in Educational Evaluation 70:101030. (26 studies, 2010–2019; outcomes largely student self-report.) https://doi.org/10.1016/j.stueduc.2021.101030
26. Gibbs G, Simpson C (2004). Conditions under which assessment supports students' learning. Learning and Teaching in Higher Education 1:3–31. (Assessment governs what and how students study — the mechanism warrant for scoring assessment design rather than curriculum statement.)
27. Kuh GD (2008). High-Impact Educational Practices: What They Are, Who Has Access to Them, and Why They Matter. AAC&U. (Internships, capstones, undergraduate research, collaborative and community-based learning; shared features include sustained time and effort, feedback, and application in novel settings.)
28. Jackson D, Collings D (2018). The influence of work-integrated learning and paid work during studies on graduate employment and underemployment. Higher Education 76:403–425. (WIL did NOT raise full-time employment rates; evidence for better relevance and quality of employment — the honest bound on the W3 claim.) https://doi.org/10.1007/s10734-017-0216-z
29. Jackson D (2016). Re-conceptualising graduate employability: the importance of pre-professional identity. Higher Education Research & Development 35(5):925–939. (Identity formed through participation in communities of practice — the mechanism distinguishing W3 from W2.) https://doi.org/10.1080/07294360.2016.1139551
30. Australian Universities Accord Final Report (2024), Department of Education. (WIL as national priority; placement poverty and the Commonwealth Prac Payment — the equity limitation declared against W3.) https://www.education.gov.au/australian-universities-accord/resources/final-report
31. NACE Career Readiness Competencies (rev. April 2024). Eight competencies: career & self-development, communication, critical thinking, equity & inclusion, leadership, professionalism, teamwork, technology. https://www.naceweb.org/career-readiness/competencies/career-readiness-defined
32. Heckman JJ, Stixrud J, Urzua S (2006). The effects of cognitive and noncognitive abilities on labor market outcomes and social behavior. Journal of Labor Economics 24(3):411–482. https://doi.org/10.1086/504455
33. Barrie SC (2006). Understanding what we mean by the generic attributes of graduates. Higher Education 51:215–241; and Barrie SC (2007), A conceptual framework for the teaching and learning of generic graduate attributes, Studies in Higher Education 32(4):439–458. (Stated graduate attributes are not evidence of attainment — why R2 binds hardest on W1.)
34. Tomlinson M (2017). Forms of graduate capital and their relationship to graduate employability. Education + Training 59(4):338–352. (Scoping source: social, cultural and psychological capital are student and network properties, not curriculum properties, and are therefore out of scope.)
35. Lester S, Costley C (2010). Work-based learning at higher education level: value, practice and critique. Studies in Higher Education 35(5):561–575. (Work-based learning in the learner's own employment as an established higher-education mode for in-practice cohorts — the warrant for W3's own-practice route, added in v4.2.) https://doi.org/10.1080/03075070903216635
`,
};

export default content;
