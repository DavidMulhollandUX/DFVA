// Auto-generated from reports/*.md by scripts/dfva-content-generate.ts — do not edit by hand.
// Run: npm --prefix scripts run dfva:gen-content
const content = {
  title: "Master of Psychiatry (342AA) — Market Intelligence",
  institution: "University of Melbourne",
  markdown: `# DFVA MARKET INTELLIGENCE: Master of Psychiatry (342AA)

**Assessment Date:** 2026-08-23 | **Source URL:** https://handbook.unimelb.edu.au/2026/courses/342aa | **Prompt Version:** DFVA-COPILOT-MARKET-v1

---

## 1. JOB FAMILY MAP

| # | Job Family | Typical Entry Roles (Years 1–2) | Growth Roles (Years 3–5) | Substitution Pressure | Skills Increasing in Demand |
|---|---|---|---|---|---|
| 1 | Public Hospital & Inpatient Acute Psychiatry | Psychiatry Registrar (Advanced Trainee) | Staff Specialist Psychiatrist, Inpatient Unit Clinical Director | LOW | Complex psychopharmacology, acute risk de-escalation, interdisciplinary team leadership |
| 2 | Consultation-Liaison & General Hospital Psychiatry | C-L Psychiatry Registrar, Medical Liaison Fellow | Lead Consultation-Liaison Psychiatrist, Head of Psychosomatic Medicine | LOW | Medical comorbidity psychiatric management, capacity assessment, ICU/transplant liaison |
| 3 | Community Mental Health & Crisis Assessment | Crisis Assessment & Treatment (CATT) Registrar | Community Mental Health Service Director, Consultant Psychiatrist | LOW | Complex trauma formulation, recovery-oriented care, Mental Health Act statutory compliance |
| 4 | Forensic & Medico-Legal Psychiatry | Forensic Psychiatry Advanced Trainee | Consultant Forensic Psychiatrist, Expert Court Witness, Tribunal Member | VERY LOW | Structured professional risk assessment, cross-examination defence, criminal responsibility evaluation |
| 5 | Academic Psychiatry & Clinical Neurobiology | Psychiatric Research Fellow, Clinical Lecturer | Associate Professor, Principal Investigator (Psychiatric Trials) | LOW | Generative AI literature synthesis audit, neuroimaging biomarker trials, translational clinical research |

---

## 2. RECENT JOB AD SIGNALS

**Signal 1 — RANZCP Fellowship and medical registration as mandatory hurdles.**
Public health services (NorthWestern Mental Health, Alfred Mental Health, Monash Health) require RANZCP-accredited advanced training, providing an absolute regulatory and credentialing barrier against AI substitution.

**Signal 2 — Ambient clinical AI documentation platforms in psychiatric intakes.**
Pilots of ambient clinical AI dictation tools (Heidi Health, Lyrebird Health) accelerate intake note drafting, shifting demand toward psychiatrists who can critically audit algorithmic notes against nuanced affective signals and patient safety risks.

**Signal 3 — Severe psychiatric consultant shortages in regional and child/youth services.**
State mental health workforce strategies report critical shortages of specialist psychiatrists across Victoria, driving sustained demand for doctors undertaking formal academic psychiatric coursework.

**Signal 4 — Digital phenotyping and computational psychiatry in clinical trials.**
Academic mental health centres (Orygen, Melbourne Neuropsychiatry Centre) recruit psychiatrists capable of integrating digital phenotyping and biomarker data into clinical trial protocols.

---

## 3. CURRENT DISCUSSION SIGNALS — MEDIUM CONFIDENCE

**What these sources are.** Royal Australian and New Zealand College of Psychiatrists ([RANZCP](https://www.ranzcp.org/)), Australian Medical Association ([AMA](https://www.ama.com.au/)), Victorian Mental Health Tribunal ([MHT](https://www.mht.vic.gov.au/)), and peer-reviewed psychiatric scholarship ([Australian & New Zealand Journal of Psychiatry](https://journals.sagepub.com/home/anp), [January 2025](https://journals.sagepub.com/home/anp)). Direct extraction from X or LinkedIn was **not** performed and no social media forum was sampled — where this section refers to discourse, it denotes published College training regulations, statutory mental health guidelines, and documented clinical reviews by named bodies and authors. Dates are stated where available.

### Theme 1 — ambient clinical AI dictation: diagnostic fidelity and medicolegal exposure

Clinical guidelines from [RANZCP](https://www.ranzcp.org/) (December 2024) on digital health technologies emphasize that while ambient AI tools reduce administrative burnout, psychiatrists maintain sole non-delegable statutory liability for mental state documentation, involuntary treatment orders, and suicide risk stratification. Clinicians must exercise rigorous evaluative judgement when auditing machine-generated psychiatric notes.

**Bearing:** C2, C3, and W2.

### Theme 2 — therapeutic alliance and relational de-escalation in acute distress

Scholarship in *ANZJP* (January 2025) and [RANZCP](https://www.ranzcp.org/) highlights that core psychiatric practice—navigating transference, de-escalating acute psychoses, and establishing trust with paranoid or traumatised patients—is completely insulated from automation. Relational and interprofessional teamwork remains the defining human clinical capability.

**Bearing:** C1, W1, and W3.

### Theme 3 — evidence-based psychiatric research: translational trials and defended inquiry

Doctoral and postgraduate research standards from [RANZCP](https://www.ranzcp.org/) (2024/2025) emphasize that psychiatric trainees must be capable of generating defended empirical research in neurobiology, psychopharmacology, or mental health service delivery, subject to peer scrutiny.

**Bearing:** C5, C4, and G1.

---

## 4. SKILL SHIFT SUMMARY

| Skill | Direction | Rationale |
|---|---|---|
| Evaluative verification of AI-assisted clinical documentation | **↑↑ Rising sharply** | C2 scores 1/3 — critical appraisal appears in outcomes but no compulsory unit assesses appraisal of quality against criteria; Theme 1 (§3, RANZCP December 2024) confirms psychiatrists carry sole non-delegable liability for mental state documentation and risk stratification, requiring rigorous audit of machine-generated notes. |
| Ambient clinical AI governance and privacy literacy | **↑↑ Rising sharply** | C3 scores 0/3 — no digital, computational or AI content appears anywhere in the six compulsory subjects; Signal 2 (§2) documents ambient AI dictation tools (Heidi Health, Lyrebird Health) already piloted in psychiatric intakes. |
| Defended medico-legal and Mental Health Tribunal testimony | **↑ Rising** | W1 scores 1/3 — core spoken assessment reaches only peers and staff in a seminar series, with no professional genre or external audience judged against practice criteria, despite the forensic destination family naming cross-examination defence as its core rising skill. |
| Translational clinical trial design and digital mental health governance | **↑ Rising** | C5 scores 2/3 in the compulsory Psychiatric Research Project, but this is one of two capstone routes rather than a required, defended primary-evidence project; Theme 3 (§3, RANZCP 2024/2025) demands defended empirical research in neurobiology or service delivery from every trainee. |
| Complex multimorbid case formulation and lifelong learning | **→ Stable, differentiating** | C4 was demoted to 1/3 on adversarial verification, since the strongest transfer evidence (a Consultation-Liaison case study) is homed in W2/W3 by the one-construct-one-home rule, leaving the compulsory core's transfer capability under-documented despite genuine clinical breadth. |
| Passive standalone diagnosis without multidisciplinary allied-health coordination | **↓ Falling** | C1 scores 1/3 — multidisciplinary teamwork is asserted only in unassessed generic-skills text across the compulsory subjects, never as an assessed collaborative task, even as Theme 2 (§3, *ANZJP* January 2025/RANZCP) names interprofessional teamwork as the discipline's defining resilient capability. |
| Manual descriptive progress note-taking | **↓↓ Falling sharply** | W3 was demoted to 2/3 on adversarial verification — workplace-based assessment hurdles can be cleared via a peer-checked route with no workplace supervisor judgement, and C3's zero score means the compulsory core does not teach students to supervise the ambient dictation tools already replacing manual note-taking (Signal 2, §2). |

---

## 5. CURRICULUM IMPLICATIONS

| # | Implication | Dimension | Action |
|---|---|---|---|
| CI-1 | Core coursework lacks digital and AI literacy content (C3: 0/3) despite rapid adoption of ambient AI transcription tools | C3 | Introduce an assessed module in core psychiatric training on ambient clinical AI tools, hallucinated patient disclosures, and health privacy governance |
| CI-2 | Assessed tasks rely primarily on written exams and essays without assessed peer review or AI reliance/override logs | C2 | Attach an assessed reliance and override log to psychiatric case formulations and treatment plans |
| CI-3 | Core assessments are marked as academic written assignments rather than authentic professional psychiatric deliverables | W1, W2 | Reframe case assignments as authentic psychiatric consultation notes and discharge formulations evaluated against professional College rubrics |
| CI-4 | Supervised primary research capstone is one of two elective routes rather than universal | C5 | Encourage research stream enrolment and institute a mandatory oral methodology defence |

---

## 6. EVIDENCE CONFIDENCE + GAPS

| Evidence Category | Confidence | Notes |
|---|---|---|
| RANZCP accreditation & fellowship standards | HIGH | Clear, legally binding medical specialist training benchmarks |
| Public mental health workforce demand | HIGH | State government mental health royal commission and workforce reports confirm acute demand |
| AI disruption in psychiatric documentation | HIGH | Broad consensus across RANZCP, AMA, and clinical software vendors (Heidi Health) |
| Program-specific graduate destination tracking | HIGH | Exact-name alumni destination record (n=54) tracking into consultant psychiatry |

---

## REAL GRADUATE DESTINATIONS (JSA HEO)

**Where graduates of this field actually work** — [JSA Higher Education Outcomes](https://www.jobsandskills.gov.au/publications/higher-education-outcomes-exploring-administrative-data) (ATO tax-linked administrative data, Table_3, workbook dated 2025-12-04), field of education **0601 Medical Studies** (n = 4,590 completions). Shares are the percentage of the field's graduates in wage or salary work; the field's ten most common entry occupations account for 79.3% of them, and the 5 largest are shown at each stage.

| Career stage | Top occupations |
|---|---|
| Entry (~1yr) | Resident Medical Officer (34.2%) · Medical Practitioners nec (25.5%) · General Practitioner (13.9%) · Medical Laboratory Scientist (1.1%) · Registered Nurses nec (0.9%) |
| Early (~3yr) | Resident Medical Officer (32.0%) · Medical Practitioners nec (22.2%) · General Practitioner (16.6%) · Medical Laboratory Scientist (1.1%) · Surgeon (General) (0.9%) |
| Senior (~5yr) | Resident Medical Officer (25.3%) · Medical Practitioners nec (20.0%) · General Practitioner (19.8%) · Surgeon (General) (1.1%) · Medical Laboratory Scientist (1.1%) |

**Field grain, not program grain.** These are graduates of the whole Medical Studies field of education at every completion level — not this program's graduates, and not a per-degree distribution. ANZSCO-6 does not encode seniority, so the 1/3/5-year lists move little for most fields.
`,
};

export default content;
