...
const content = {
  title: "Master of Evaluation (MC-EVALO) — Durability Report (v4 draft)",
  institution: "University of Melbourne",
  markdown: `# DFVA v4 DURABILITY REPORT: Master of Evaluation (MC-EVALO)
...
> **This program is outside the v3.1 registry, but not outside Panel A.** Master of Evaluation
...
**Destination AI Exposure: 96.58.** Destination titles are mapped to the published Felten AI Occupational Exposure (AIOE) index and rescaled 0–100 by the Panel A procedure used for every other program; 6 of 13 titles carry medium mapping confidence. Basis: measured on the program's own alumni destination record (n = 86, 13 titles). The portfolio median is 90.9, so this program sits **above** it.
...
and the score deltas are in the [improvement plan](dfva-v4-recommend-mc-evalo.md); rows
...
**Panel A metadata.** Basis tier **exact** (measured), n = 86, 13 destination titles, 6 at medium mapping confidence. Index variant AIOE-2021; destination titles mapped to the published Felten index.
