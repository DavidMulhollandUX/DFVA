...
export class OpenAiLlmScorer implements LlmScorer {
  async score(pages: HandbookPages): Promise<AssessmentResult> {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are the Degree Future-Viability Assessor. Return only valid JSON. Score honestly against the rubric — do not inflate.",
          },
    ...
    if (
      ...
      result.dimensions.length < 10
    ) {
      throw new Error(
        `Invalid assessment: score=${result.score}, dimensions=${result.dimensions?.length}`,
      );
    }
