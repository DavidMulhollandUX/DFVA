...
test.describe('/insights — v4 portfolio overview', () => {
  test('renders 118 rows: 104 assessed and 14 research degrees', async ({ page }) => {
    await page.goto('/insights');
    const rows = page.locator('[data-testid="program-row"]');
    await expect(rows).toHaveCount(118);
    await expect(page.locator('[data-assessed="true"]')).toHaveCount(104);
