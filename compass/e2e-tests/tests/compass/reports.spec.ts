...
test.describe("/reports/:slug — dispatcher", () => {
  ...
  test("a scored program links its archived v1 reports", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="archived-reports"]')).toHaveCount(
      1,
      LOAD,
    );
  });

  test("an archived (v1-only) program shows the pending state with an archived link", async ({
    page,
  }) => {
    ...
    test.skip(
      (await archived.count()) === 0,
      "no v1-only program left in the index",
    );
    await page.getByLabel("Status").selectOption("archived");
    await page
      .locator('[data-testid="durability-report-link"]')
      .first()
      .click();
    await expect(page).toHaveURL(/\/reports\/[^/]+$/);
    await expect(page.locator('[data-testid="v4-pending-notice"]')).toHaveCount(
      1,
      LOAD,
    );
    await expect(page.locator('[data-testid="archived-v1-link"]')).toHaveCount(
      1,
      LOAD,
    );
  });
  ...
  test("a dfva-* slug renders the archived v1 page with its banner", async ({
    ...
    await expect(
      page.locator('[data-testid="archived-report-banner"]').first(),
    ).toBeVisible(LOAD);
  });

  test("a research degree renders the research notice and no v1 vocabulary", async ({
    page,
  }) => {
    await page.goto("/reports/dr-philsci");
    await expect(
      page.locator('[data-testid="v4-research-notice"]').first(),
    ).toBeVisible(LOAD);
    await expect(page.getByTestId("v4r-no-plan")).toBeVisible(LOAD);
