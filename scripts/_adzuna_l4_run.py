...
"""Adzuna AU L4 fetch for the 4-SOC batch run (2026-08-30).

Uses ADZUNA_APP_ID / ADZUNA_API_KEY from the environment (present on this host).
Writes raw + summary per SOC and folds a jobAds block into each ledger.
This is the quantitative L4 source (time-series all-time index count + top
employers/skills). It does NOT replace LinkedIn (who advertises, live postings)
or Seek, but both are attempted separately.
"""
...
JOBS = {
    "47-1011": ("construction supervisor",
                ["site safety", "subcontractor", "scheduling", "tender", "quality",
                 "WHS", "budget", "compliance", "project delivery", "supervision"]),
    "47-2061": ("construction labourer",
                ["manual handling", "site clean", "demolition", "concrete", "labouring",
                 "construction", "safety", "physical", "plant", "tools"]),
    "47-2111": ("electrician",
                ["electrical", "wiring", "test and tag", "switchboard", "compliance",
                 "license", "solar", "maintenance", "fault finding", "ASIC"]),
    "49-2094": ("electronic equipment trades worker",
                ["electronics", "repair", "maintenance", "PLC", "instrumentation",
                 "industrial", "fault finding", "test", "commissioning", "controls"]),
}
...
def summarise(resp: dict, lexicon: list[str]) -> dict:
    ...
    if not top_skills:
        # Surface the raw most-common skill-like tokens minimally as a gap marker.
...
def main() -> int:
    ...
    for soc, (query, lexicon) in JOBS.items():
        ...
        try:
            ...
            # Record as a coverage gap, do not fabricate.
        ...
        # Fold into ledger jobAds block (merge: preserve other jobAds keys).
        ledger = REPO / "data" / "professions" / f"{soc}.json"
        d = json.loads(ledger.read_text())
